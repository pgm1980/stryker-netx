# Bug-Report: `dotnet-stryker-netx` (v3.0.24 → … → v3.2.5) — Verlaufs-Dokumentation

**An:** stryker-netx Maintainer-Team
**Von:** Calculator-Testbed-Projekt (.NET 10 / C# 14)
**Datum:** 2026-05-06 (Erstausgabe v3.0.24, Updates für v3.1.1, v3.2.0, v3.2.1, v3.2.5)
**Status (v3.2.5):** **eingeschränkt produktionsreif.** Maintainer-Team hat Forderungen-Sektion aufgegriffen — Bugs #4, #6, #8 sind gefixt; Bug #9 ist auf Domain gefixt, aber **das geforderte Audit (Punkt e) wurde nicht umgesetzt**: auf Calculator.Infrastructure crasht das `All`-Profile mit einem _zweiten_, strukturell identischen Cast-Fehler (`ParenthesizedExpressionSyntax → IdentifierNameSyntax` statt `→ TypeSyntax`). Der Fix war punktuell statt systemisch.
...
| Bug | v3.0.24 | v3.1.1 | v3.2.0 | v3.2.1 | v3.2.5 |
|-----|---------|--------|--------|--------|--------|
| **#1** Profile-Flag | 🔴 | ✅ | ✅ | ✅ | ✅ |
| **#2** Banner | 🟡 | ✅ | ✅ | ✅ | ✅ |
| **#3** Update-Hinweis | 🟡 | ✅ | ✅ | ✅ | ✅ |
...
| **#5** Analysis-Warning | 🟡 | ✅ | ✅ | ✅ | ✅ |
| **#6** `--reporters` | 🟡 | ❌ | ❌ | ❌ | ✅ |
| **#7** NuGet-Push | 🟢 | n/a | 🟢 | ✅ | 🟢 (gepusht, im Search-Index verzögert) |
| **#8** Multi-Project-UX | 🟢 | ❌ | ❌ | ❌ | ✅ (`--all-projects` neu) |
| **#9** All-Crash | — | 🔴 | 🔴 | 🟠 (transformiert) | 🟠 **partiell**: Domain ✅, Infrastructure 🔴 anderer Cast |
...
### Zusammenfassung der Forderungen — Erfüllungsstand v3.2.5

| Priorität | Bug | Fix-Erwartung | Erfüllungsstand v3.2.5 |
|-----------|-----|---------------|------------------------|
| **P0** | #9 | Tiefer architektureller Eingriff (a-e), kein Hotfix | 🟠 **Punkte (a-d) augenscheinlich umgesetzt; Punkt (e) Audit nicht durchgeführt** — zweite Cast-Stelle existiert weiter |
| P1 | #4 | `--version` als Tool-Version-Flag etablieren | ✅ Erfüllt |
| P1 | #6 | `--reporters` (plural) als Alias akzeptieren | ✅ Erfüllt |
| P1 | #8 | `--all-projects` oder Mehrfach-`--project`-Angabe | ✅ Erfüllt — sogar exakt in vorgeschlagener Form |

**Bilanz:** Drei von vier Forderungen vollständig erfüllt. Bei der vierten — der einzigen P0-Forderung — wurde 4 von 5 Unterpunkten umgesetzt, aber ausgerechnet der wichtigste (Audit) übersprungen. **Die Forderung bezüglich Bug #9, Punkt (e), bleibt mit verschärftem Nachdruck bestehen** — das Wiederauftreten des strukturell identischen Bugs an einer zweiten Stelle ist der lebende Beweis dafür, dass das Audit notwendig ist.

Wir sind weiterhin bereit, die Repro-Codebase und den Branch `feature/2.5-stryker-v3.2.5-verification` als Test-Subjekt zur Verfügung zu stellen. Eine zeitnahe Rückmeldung zum geplanten Vorgehen für den Audit würden wir begrüßen.
