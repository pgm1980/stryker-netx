...
function Invoke-Stryker {
  ...
  try {
    if (-not [string]::IsNullOrWhiteSpace($StrykerPath)) {
      Write-Info "Running local dotnet-stryker-netx in '$RunDirectory'"
      Write-Info ("Run command: " + (($StrykerPath + ' ' + (($effectiveToolParameters | ForEach-Object { $_ }) -join ' ')).Trim()))
      & $StrykerPath @effectiveToolParameters
    } else {
      $dotnetInvocation = @('tool', 'run', 'dotnet-stryker-netx') + $effectiveToolParameters
      Write-Info "Running dotnet-stryker-netx (from tool manifest) in '$RunDirectory'"
      Write-Info ("Run command: " + (((@('tool', 'run', 'dotnet-stryker-netx') + $effectiveToolParameters) | ForEach-Object { $_ }) -join ' '))
  ...
  if ($LASTEXITCODE -ne 0) { throw "dotnet-stryker-netx failed with exit code ${LASTEXITCODE}" }
}
...
function Pack-And-Install-Tool {
  ...
  dotnet tool uninstall dotnet-stryker-netx --tool-path $ToolPath 2>$null
  if ($LASTEXITCODE -ne 0) { Write-Info "dotnet-stryker-netx was not previously installed in '$ToolPath' (continuing)" }

  dotnet tool install dotnet-stryker-netx --add-source $PublishPath --allow-downgrade --tool-path $ToolPath --version $ToolVersion
...
if ($useLocalTool) {
  ...
  $toolProject = Join-Path $repoRoot 'src' 'Stryker.CLI' 'Stryker.CLI.csproj'
