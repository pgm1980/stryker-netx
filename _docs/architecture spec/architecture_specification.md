...
**Status:** Accepted · **Kontext:** Dritter und LETZTER Sprint des externen-360°-Test-Fix-Blocks (185–187). INJ-001 (Report): `MethodBodyReplacementMutator` + `GenericConstraintMutator` (beide `MethodDeclarationSyntax`) lassen sich nicht injizieren → still zu CompileError; 2 `All`-Operatoren funktionslos. Architektur-Analyse via MAXential+ToT + Live-Probe (bug02). Backed by: Inject-Mechanik (RoslynHelper.TryInjectMutation prüft `sourceNode.Contains(OriginalNode)`; höchster Frame = Method-BODY via BaseFunctionOrchestrator; Inject-Overloads nur Expression/Statement/Block).
