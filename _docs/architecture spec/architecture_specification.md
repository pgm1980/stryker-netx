...
## ADR-051: Dogfood-Configs netx-Layout + Fixture-Restore-Pflicht + NetFramework-Matrix honest-deferred (Sprint 171, kein Tag)

**Status.** Accepted — Sprint 171 (2026-06-11). **Kein Version-Tag** (Sprint-138-Präzedenz: ausschließlich CI-Skripte, Workflows, Fixture-Configs und Tool-Manifest — kein shipped Code).

**Kontext.** Sprint-170-Follow-ups (Issue #268, dokumentiert in ADR-050): Der erste funktionierende Dispatch-Run des Nightly (27339863955) und die Census-Analyse der Integration-Matrix (Run 27339543191: 27 netcore-Failures + 2 netframework-Failures + InitCommand 3× grün) legten drei getrennte Schuldenschichten frei.

### Entscheidungen

**E1 — Dogfood-Configs (Item 1).** Alle `src/*/stryker-config.json` waren Sprint-24-Vendoring mit Upstream-Test-Projektpfaden (`../*.UnitTest.csproj` bzw. `../Stryker.Core/Stryker.Core.UnitTest/`), die im netx-Layout nie existierten; `Stryker.Solutions` und `Stryker.TestRunner` hatten GAR KEINE Config (Matrix-Jobs liefen in Default-Discovery → Fehlschlag). Fix: 9 Configs umgestellt + 2 neu, Mapping nach Referenz-Matrix (Test-csproj → src-Modul, Upstream-Philosophie gespiegelt):

| Modul | test-projects (netx) | Begründung |
|---|---|---|
| Abstractions, Configuration, Utilities | Core.Dogfood.Tests | Upstream zeigte auf Core.UnitTest; Dogfood.Tests IST die portierte Upstream-Suite |
| Core | Core.Dogfood.Tests + Core.Tests | Dogfood = Upstream-Parität; Core.Tests (492 Mutator-Tests) = Kill-Power auf Mutator-Code |
| CLI | CLI.Tests | direkter Mirror |
| RegexMutators | RegexMutators.Tests | direkter Mirror |
| DataCollector, TestRunner.VsTest | TestRunner.VsTest.Tests | exakter Upstream-Mirror (upstream: TestRunner.VsTest.UnitTest) |
| TestRunner.MTP | TestRunner.MTP.Tests (+ `test-runner: mtp` beibehalten) | direkter Mirror |
| Solutions (NEU) | Solutions.Tests | fehlte komplett |
| TestRunner (NEU) | TestRunner.MTP.Tests + TestRunner.VsTest.Tests | Abstraktionsschicht wird durch beide Adapter exerziert |

Zusätzlich `project-info.name` überall `github.com/stryker-mutator/stryker-net` → `github.com/pgm1980/stryker-netx` (Dashboard-Slug-Korrektheit). Verifiziert: `--break-after analysis` EXIT=0 auf allen 11 Modulen, 0 Treffer für „produced no source projects / can't be mutated / Failed to analyze".

**E2 — Fixture-Restore-Pflicht (Item 2, Root-Cause netcore).** `integration-tests.ps1` packt nur die CLI und restauriert die Fixtures nie. MSBuildWorkspace-Analyse scheitert, wenn REFERENZIERTE Source-Projekte kein `obj/project.assets.json` haben: Source-Project-Filter findet nichts („Project filter 'TargetProject.csproj' produced no source projects") → „Failed to analyze project builds". **Lokal bewiesen durch Repro-Paar:** Fixture mit bin/obj → grün; nach `git clean -xdf` → CI-identischer Fehler; nach `dotnet restore` der Fixture-Solution → grün (beide Pfade: NetCore-Fixture + MTP-Fixture). Empirische Nuance: Ein unrestauriertes TEST-Projekt wird toleriert, solange die referenzierten Source-Projekte restauriert sind (deshalb traf es den Nightly nicht: CLI-Pack restauriert src/* transitiv) — undokumentiertes Verhalten, auf das man nicht baut. Fix: `Restore-Fixture`-Helper + per-Kategorie-Restore (NetCore-Kategorien → `IntegrationTestApp.sln`, MTP-Kategorien → `MicrosoftTestPlatform.slnx`, InitCommand braucht keinen — läuft analysis-frei grün; netframework behält den dedizierten nuget.exe-Step). Verworfen: Blanket-Restore beider Solutions in Main (30 Jobs zahlen je ~30–90 s für zur Hälfte unnötige Restores); Auto-Restore in Run-Stryker (WD-Mehrdeutigkeit: TargetProjects hat mehrere csproj). Flankierend in `stryker-on-stryker.ps1`: einmaliges `dotnet restore stryker-netx.slnx` nach dem Pack — 1-Zeilen-Versicherung gegen das Toleranz-Verhalten, wirkt in beiden Tool-Modi.

**E3 — NetFramework-Matrix honest-deferred (Item 2, Root-Cause netframework).** Andere Fehlerklasse: `Microsoft.CodeAnalysis.MSBuild.RemoteInvocationException → TypeInitializationException 'Microsoft.Build.Shared.XMakeElements'` — MSBuildWorkspace kann Legacy-non-SDK-csproj nicht laden (SDK-MSBuild via MSBuildLocator vs. Full-Framework-MSBuild-Anforderung alter Projektformate). Kein Restore-Thema; echte Reparatur ist Tool-Level-Arbeit und berührt die ADR-002-Frage, ob net48-Targets first-class bleiben. Entscheidung: `continue-on-error: ${{ matrix.runtime == 'netframework' }}` — die 2 Jobs laufen sichtbar weiter (rote Annotation), blockieren aber Workflow + Fan-in nicht mehr; die 28 funktionierenden netcore/MTP-Jobs gewinnen ihr Regressions-Signal zurück. Verworfen: rot lassen (Matrix bleibt dauerrot = Signal wertlos, die ADR-050-Falle invertiert); aus Matrix entfernen (Support-Drop ohne Entscheidung, Signal weg).

**E4 — Manifest-Nachzug + Auto-Bump verworfen (Item 3).** `.config/dotnet-tools.json` 3.3.1 → 3.3.2 (NuGet-Indexierung via WebFetch bestätigt, `dotnet tool restore` verifiziert). release.yml-Auto-Bump VERWORFEN (Maxential 3 Thoughts): ungated Push auf main bricht die Squash-PR-Konvention; Indexing-Lag erzeugt ein automatisiertes Chicken-Egg (ADR-050 D2 vermied genau das als Option C); zusätzliche Failure-Modes im kritischsten Workflow — gegen die Ersparnis eines 1-Zeilen-Edits pro Release. Stattdessen Prozess-Konvention: Pin-Nachzug nach verifiziertem Release + bestätigter Indexierung im jeweils nächsten PR.

### Files

- EDIT 9× + NEU 2× `src/*/stryker-config.json` (E1)
- EDIT `integration-tests.ps1` — Restore-Fixture-Helper + 9 per-Kategorie-Aufrufe (E2)
- EDIT `stryker-on-stryker.ps1` — Solution-Restore nach Pack (E2)
- EDIT `.github/workflows/integration-test.yaml` — continue-on-error für netframework (E3)
- EDIT `.config/dotnet-tools.json` — Pin 3.3.2 (E4)

### Scope-Grenze

Erfolgskriterium Integration-Matrix: netcore/MTP-Kategorien (28 Jobs) kommen über die Analysis-Phase hinaus; Workflow + Fan-in grün. Erfolgskriterium Nightly: Module kommen über die Analysis in die Mutation-Loop. **NICHT Kriterium:** Mutation-Scores oder vollständige 120-Min-Läufe aller Module (Erstläufe können weitere Schichten zeigen — dann eigene Sprints).

### Konsequenzen

- (+) Integration-Matrix erstmals mit nutzbarem Signal (28/30 verbindlich, 2 sichtbar-geduldet).
- (+) Nightly-Dogfood hat erstmals korrekte Modul-Configs — echte Mutation-Runs möglich.
- (+) Dashboard-Slugs zeigen auf das richtige Repo.
- (–) NetFramework-Kategorien bleiben ungelöst (Tool-Level-Backlog; Verbindung zu ADR-002).
- (–) Manifest-Pin braucht weiterhin manuellen Nachzug pro Release (bewusste Entscheidung E4).

### Backlog-Notiz (nicht Sprint 171)

Tool-seitiger Auto-Restore vor der Analysis, wenn `project.assets.json` der beteiligten Projekte fehlt — Produktverbesserung für CI-First-Nutzung (Fresh-Checkout-Pipelines treffen exakt den E2-Fehler mit opaker Meldung „Failed to analyze project builds"). Upstream-Parität: Buildalyzer-Design-Time-Builds deckten das implizit ab; unser MSBuildWorkspace-Ansatz (Phase 9) verlangt restaurierte Projekte. Kandidat für die anstehende Aufgaben-Priorisierung.

**Backed by.** Sprint 171 Maxential-Sessions (Item 3: 3 Thoughts; Item 2: 4 Thoughts). Lokal verifiziert: 11/11 Module `--break-after analysis` EXIT=0; Fixture-Repro-Paar clean→rot / restored→grün auf NetCore- UND MTP-Pfad; `dotnet tool restore` → 3.3.2.

### Nachtrag (gleicher Sprint): Dispatch-Verifikation legt Tool-Bug im ADR-039-Layer-3-Gate frei

Der Post-Merge-Dispatch-Run 27345502390 ergab **3 Module mit erstmals vollständigen Mutation-Runs** (RegexMutators, Solutions, TestRunner.VsTest — Projekthistorien-Premiere) und **8 Failures** mit einheitlichem Muster:

```
WRN Project filter '<Modul>' produced no source projects. Retrying with all source projects.
…
Test project contains more than one project reference. Please set the project
```

**Diagnose-Kette (Theorie aufgestellt → falsifiziert → verfeinert):**
1. Hypothese „Linux vs Windows" — falsch: Windows-Repro nach `git clean -xdf src/ tests/` + restore-only zeigt identisches Verhalten.
2. Hypothese „restauriert-aber-ungebaut" — **falsifiziert**: Auch nach erfolgreichem `dotnet build stryker-netx.slnx` (0/0) feuert der Fallback unverändert.
3. **Befund:** `ApplyProjectFilter` (InputFileResolver.cs:475–484, ADR-039 Layer 3) zählt `sourceCount` über `BuildsAnAssembly()` (= `TargetFileName` nicht leer, RoslynProjectAnalysis.cs:288). `TargetFileName` ist in der Design-Time-Analyse dieses Szenarios (Test-Projekt-Modus mit `project`-Filter) offenbar GENERELL leer ⇒ das Layer-3-Safety-Net verwirft den korrekt matchenden Filter IMMER ⇒ der `project`-Key der Dogfood-Configs wird de facto ignoriert ⇒ Multi-Referenz-Module sterben an der Disambiguierung; Single-Referenz-Module überleben nur, weil der Fallback genau einen Kandidaten findet. Auch die WRN-Botschaft ist irreführend (der Filter HAT gematcht — das Assembly-Kriterium hat verworfen). Offene Frage für den Fix: warum TestRunner.VsTest (3 Referenzen) durchlief — vermutlich BuildsAnAssembly-Varianz pro Analysis-Pfad.
4. **Verifikations-Blindfleck dokumentiert:** Die 11/11-`--break-after analysis`-Prüfung dieses Sprints war unzureichend — break-after exitet VOR der Disambiguierung (Exit 0), und das `tail -6`-Capture-Fenster verbarg die frühe WRN. Lehre: Filter-Korrektheit braucht einen Voll-Lauf-Probe ODER explizites Grep auf die WRN über den GESAMTEN Output.

**Scope-Entscheidung:** Tool-Fix ist Stryker.Core-Produktarbeit (Layer-3-Gate-Logik + TargetFileName-Population + WRN-Text) — per Scope-Grenze dieses ADRs („Erstläufe können weitere Schichten zeigen — dann eigene Sprints") als **Sprint-172-Kandidat** ausgegliedert, GitHub-Issue mit vollständiger Repro dokumentiert. Die Sprint-171-Artefakte (Configs, Restores, continue-on-error) sind unabhängig davon korrekt und notwendig; Integration-Matrix-Fan-in wurde grün (PR #269: 30 Passes + 2 allowed-failures).
...
## ADR-053: Quick-Win-Fixes aus dem 360°-Analyse-Programm (v3.3.4 / Sprint 179)

**Status:** Accepted · **Kontext:** Erster Fix-Sprint des Fahrplans aus `_docs/analysis/sprint_178_synthesis.md` (Programm 173–178, 139 Findings). Scope: alle Einzeiler-Klasse-Fixes plus der P1.

**Entscheidungen & Fixes (TDD: Red-Test aus dem Register-Befund zuerst, je Fix verifiziert):**

1. **#282 P1 (G-25):** C#-14-typlose out-Lambda-Parameter (`Type == null`) crashten den GESAMTEN Lauf mit unhandled NRE in `DefaultInitializationEngine`. Fix: typloses `default`-Literal als Fallback. Probe P-4 gegen lokale CLI: Exit 127 → 0.
2. **#294 (I-02):** `TestIdentifierList.Contains` war invertiert (`is false`) — ruhende Landmine ohne Live-Aufrufer und ohne Tests. Fix: `is true` + 6 Vertragstests der Mengen-API inkl. WrappedIdentifierEnumeration-Delegation (Red 5/6 vor Fix).
3. **#295 (I-08):** `DetectTestFrameworks` LÖSCHTE das MsTest-Flag (`&= ~` statt `|=`) — MsTest-Suiten bekamen nie `<DisableParallelization>`. Fix + RunSettings-Test.
4. **#283 (G-14):** case-Labels/goto-case als 5. Konstanten-Kontext gefenced (`DoNotMutateOrchestrator<CaseSwitchLabelSyntax>` + goto-case-Prädikat). Trade-off dokumentiert: Statement-Removal-Mutanten auf goto-case entfallen. Probe-1: Case-Label-CE-Klasse vollständig eliminiert (Mutanten entstehen gar nicht erst; Datei-CE-Rate 70 % → 62,5 %). MA0051-Split: `NonMutableSyntaxFences()`.
5. **G-01-MECHANIK-KORREKTUR (#279):** Der Verdacht „EndingReturnEngine fehlt im Mutations-Pfad → CS0161" ist ENTKRÄFTET — der `needReturn`-Flag-Pfad von `MutationStore.Inject` leistet das Ending-Return bereits (Diagnose-Dump). Die Proben-Block-CEs waren Rollback-Kollateral der Block-first-Heuristik (G-30); echte Verursacher: #284/#278. Ein No-op-„Fix" wurde durch Red-Verifikation VOR dem Ship entdeckt und verworfen; stattdessen Pin-Test für die needReturn-Garantie. Lehre: CE-Diagnostic-Codes verifizieren, nicht nur Status; Register-Attribution per Probe gegenprüfen.
6. **#292 (H-12):** Test-Enrichment guarded gegen VsTest-Metadaten (LineNumber 0/out-of-range, Zeile ohne Methodendeklaration) — Skip+DBG statt Lauf-Abbruch.
7. **#273 (H-01):** Flaky-Test deterministisch: Untergrenze trägt die Semantik („Stopwatch maß ≥ Delay"), Obergrenze nur Absurditäts-Schutz (1..200 ms → 10..60 000 ms).
8. **#274 (I-01):** Vendored `test-runner: mtp`-Key aus der Dogfood-Config entfernt (xunit-Projekt ist nicht MTP-fähig) → MTP-Modul läuft über VSTest-Default; Nightly-Ziel 11/11.
9. **#302-Einzeiler H-05 + J-11:** `FindProjectFile`-Extension-Check war No-op (`Path.HasExtension` auf dem Literal-String) → fail-fast InputException + 4 Tests; ConfigBuilder probt den kombinierten Pfad statt des nackten Dateinamens.

**Verifikation:** Build 0/0 · Vollsuite grün · Semgrep 0 Findings auf 15 geänderten Dateien · Probe P-4 Exit 0 · Probe-1 Case-Label-CEs eliminiert. **Tag v3.3.4** (Patch — reine Bugfixes).

**Konsequenzen:** Issues #273, #274, #282, #283, #292, #294, #295 geschlossen; #279-Checkliste um die CS0161-Teilthese bereinigt; #302 um H-05/J-11 abgehakt. Sprint-180-Vorlage: #284/#278/#285 (CE-Noise I) mit Probe-Infrastruktur als Erfolgsmaß.

---

## ADR-054: CE-Noise I — Designation-Patterns & TrackValue-Marker (v3.3.5 / Sprint 180)

**Status:** Accepted · **Kontext:** Zweiter Fix-Sprint des Fahrplans (`sprint_178_synthesis.md`). Cluster „CE-Noise": drei Befundgruppen, die auf der Probe-1-Datei zusammen die 62,5-%-CE-Rate trugen. Erster Sprint der Serena-Ära (Symbol-Navigation statt Volltext-Lektüre; Memory-Update vor/nach Sprint).

**Mechanik (aus der Pipeline-Analyse, Sprint 174):** Pattern-interne Mutationen können nicht im Pattern injiziert werden (Konstanten-Kontext; `ConstantPatternSyntaxOrchestrator` blockt Inline-Injection). Sie steigen im geteilten Expression-Store zum nächsten injektionsfähigen Expression-Slot auf — dem `is`-Ausdruck. Der Ternary-Wrap dort dupliziert eine vorhandene Designation im selben Statement-Scope → CS0128. Der Block-Lift (if/else um den ganzen umgebenden Block, jede Kopie self-contained mit eigenem Scope) ist die einzig legale Kontroll-Ebene — exakt der Mechanismus, den `ContainsDeclarations` für out-vars seit Upstream nutzt.

**Entscheidungen & Fixes (TDD: Red-Test je Fix zuerst):**

1. **#278 (F-29):** `IsPatternExpressionMutator` emittiert nichts mehr, wenn das Pattern eine Variable bindet — die Negation der Wurzel bricht Definite Assignment jeder späteren Nutzung (CS0165, garantiert bei Konsum der Variable). Discard-Designationen (`is int _`) binden nichts und bleiben mutierbar. Red: 6 Theory-Fälle (Declaration/Var/Recursive/Property/List/not), 3 Mutierbarkeits-Kontrollen.
2. **#284a (G-26):** Neuer `PatternSyntaxOrchestrator` (ersetzt die generische `NodeSpecificOrchestrator<PatternSyntax, PatternSyntax>`-Registrierung) + `is`-Pattern-Kontext-Check in `ExpressionSpecificOrchestrator`: Mutationen in designation-tragenden `is`-Patterns werden zur Store-Zeit auf `MutationControl.Block` geroutet (Level-Routing via `FindControl`, nicht beim Bubbling — `Leave()` degradiert Level). Switch-Arme/Case-Labels bewusst ausgenommen: arm-/section-scoped, dort bleibt der bestehende Wrap legal.
3. **#284b:** `ContainsDeclarations` kennt jetzt `SingleVariableDesignation` — Designationen an Recursive-/Var-/List-Patterns waren dem Block-Routing unsichtbar (z. B. `&&`-Mutation auf einer Bedingung mit `o is { Length: 2 } t`). Lambda-Bodies bleiben durch `ContainsNodeThatVerifies` ausgeschlossen (lambda-scoped Designationen leaken nicht). Neue geteilte Helfer: `ContainsBindingDesignation`, `IsInsideVariableBindingIsPattern` (#278 nutzt dasselbe Prädikat).
4. **#285a (G-32):** `MutantPlacer.PlaceStaticContextMarker(Expression)` entscheidet jetzt via `CanHostValueMarker`: (i) target-typed Ausdrücke (`new()` → CS8754, Collection-Expressions, `default`-Literal) NIE wrappen — das Tracking-Lambda löscht den Zieltyp; bisher räumte erst eine Rollback-Heilrunde (mutant −1) den Marker wieder ab. (ii) Konstanten-artige Initializer OHNE Mutationen nie wrappen — nichts zu tracken, und der Wrap löscht Konstanten-Konvertierungen (`byte x = 5` → CS0266). (iii) Alles, was User-Code ausführen kann (Invocations, Member-Access, explizite Konstruktionen), behält den Marker auch ohne lokale Mutationen — `TrackValue` setzt den Static-Kontext für transitiv aufgerufene Mutanten (per Pin-Test gesichert; ein pauschaler Annotation-Check hätte das Coverage-Tracking von `= Compute()`-Initializern gekostet).
5. **#285b (H-28):** Property-Initializer-Pfad (`ExpressionBodiedPropertyOrchestrator`) auf `MustInjectCoverageLogic` gegated — der Static-Field-Pfad war es seit jeher.

**Trade-offs:** Block-Lift senkt Mutations-Granularität für Pattern-Mutanten (ganzer Block dupliziert statt Inline-Ternary) — Korrektheit vor Kompaktheit. #285a-Skip auf target-typed Initializern MIT Mutationen verliert deren Static-Tracking nicht wirklich: der Marker überlebte den Rollback heute auch nicht, gespart wird die Heilrunde samt G-30-Kollateral-Risiko. Verschachtelte target-typed Ausdrücke (Ternary aus Collection-Expressions) bleiben der Heilrunde überlassen (selten, dokumentiert).

**Dazu:** Dogfood-Manifest-Pin 3.3.2 → 3.3.4 (Konvention Pin-Nachzug, ADR-051 E4).

**Verifikation:** Build 0/0 · Vollsuite grün (519 + 1212 Core; Gesamtsuite im Ship-Gate) · Semgrep 0 Findings auf 7 geänderten Src-Dateien · Probe-1-CE-Rate 62,5 % → Ziel < 30 % (Messung im Ship-Gate) · Debug-Log ohne „mutant −1"-StaticInitializer-Heilrunde. **Tag v3.3.5** (Patch — reine Bugfixes).

**Konsequenzen:** Issues #278, #284, #285 geschlossen. Probe-1-Restklasse danach: RelationalPattern-Konstanten in designation-FREIEN Patterns (kompilieren als Ternary am `is`-Ausdruck weiter). Sprint-181-Vorlage: Score-Integrität (#286/G-17/G-19/J-01).

---

## ADR-055: Score-Integrität — Geister-Mutanten, No-op-Filter, sichtbare Filter-Entscheidungen (v3.3.6 / Sprint 181)

**Status:** Accepted · **Kontext:** Dritter Fix-Sprint des Fahrplans (`sprint_178_synthesis.md`). Cluster „Score-Integrität": vier Befunde, die den Mutation-Score leise verfälschen — nicht durch CE-Lärm, sondern durch False Survivors, unsichtbare Verluste und schema-fremde Report-Stati.

**Entscheidungen & Fixes (TDD; G-19-Red zusätzlich per Stash-Roundtrip gegen die alte Implementierung bewiesen):**

1. **G-17 — `NoOpMutationFilter` als Filter #0:** Kein bestehender Filter fing generische No-op-Emissionen (Replacement strukturell ≡ Original, z. B. TypeDrivenReturn auf bereits-Zielwert `return 0;`→`return 0;`) — garantierte False Survivors. Der neue Filter vergleicht trivia-insensitiv (`SyntaxFactory.AreEquivalent(topLevel: false)`) und läuft als ERSTER (billigster Check, breiteste Klasse; Attribution geht an „NoOpMutation" statt an teurere shape-beschränkte Filter). `IdentityArithmeticFilter` bleibt als spezifischere Defensive bestehen.
2. **G-19 — Filter-Entscheidungen sichtbar:** Equivalence-gefilterte Mutationen wurden per `continue` verworfen, BEVOR ein Mutant entstand — kein Status, kein Report-Eintrag; Filter-False-Positives = unsichtbarer Mutanten-Verlust. Jetzt: Ignored-Mutant mit `ResultStatusReason = "Equivalent mutant (filter: <FilterId>)"` (Upstream-Parität), registriert aber NIE injiziert (würde das Assembly nur aufblähen). Dedup-Check gilt auch hier. Wechselwirkung dokumentiert: No-ops erscheinen seit G-17 als Ignored im Report statt gar nicht.
3. **#286/G-15 — Geister-Mutanten → CompileError:** Das ADR-032-Sicherheitsnetz (`ReplaceChildrenValidated`) verwarf slot-inkompatible mutierte Subtrees nur mit Debug-Log; die enthaltenen, bereits registrierten Mutanten endeten als NoCoverage (Coverage-Modus) bzw. Survived (ohne Coverage). Plumbing: neuer optionaler Callback `onMutationsDropped` (beide Drop-Pfade: Per-Child-Reject + Bulk-Replace-Fallback) → `MutationContext.FlagDroppedMutantsIn` → `CsharpMutantOrchestrator.FlagDroppedMutants` (IDs via neuem `MutantPlacer.ExtractMutantIds` aus den `MutationId`-Annotationen des Subtrees; nur Pending-Mutanten werden umgesetzt, settled Results bleiben unangetastet; INFO-Log je Mutant). Alle 4 Call-Sites verdrahtet. Dazu ein generischer **Geister-Detektor** in den Slot-Validation-Integrationstests: jeder nach Orchestrierung noch Pending-Mutant MUSS seine Annotation im mutierten Baum haben.
4. **J-01 — kein „Pending" im Final-Report:** Mutanten, die nach Abschluss der Testsession Pending blieben (Executor inkonklusiv; das G-09-WARN existierte schon), liefen mit schema-fremdem Status in den JSON-Report und wurden dem Realtime-Reporting vorenthalten (`OnMutantTested` skippt Pending). Finalisierung am Drain (`OnMutantsTested`): `Ignored` + Reason „The test session could not assess this mutant" — score-neutral und ehrlich; der Mutant erreicht damit auch das Realtime-Reporting genau einmal.

**Trade-offs:** Ignored-Mutanten (G-19/J-01) zählen nicht in den Score — bewusst: „nicht bewertet/nicht bewertbar" darf weder als Killed noch als Survived gewertet werden. Der G-15-Callback ist optional (Default null) — bestehende externe Aufrufer kompilieren unverändert; nur die Pipeline-Call-Sites melden.

**Verifikation:** Build 0/0 · Vollsuite grün · Semgrep 0 auf 11 geänderten Src-Dateien · Geister-Detektor grün über alle 10 historischen Crash-Patterns. **Tag v3.3.6** (Patch).

**Konsequenzen:** Issue #286 geschlossen; G-17/G-19/J-01 im Register als gefixt markierbar. Sprint-182-Vorlage: Robustheit (#277-Schicht/#288/#297/Parse-Guards).

---

## ADR-056: Robustheit — Crash-, Hang- und Parse-Härtung (v3.3.7 / Sprint 182)

**Status:** Accepted · **Kontext:** Vierter Fix-Sprint des Fahrplans (`sprint_178_synthesis.md`). Cluster „Crash/Hang-Robustheit": Ein Tool, das auf User-Code und fremde Infrastruktur losgelassen wird, darf weder an einem einzelnen Mutator-Bug sterben noch an toter Infrastruktur ewig hängen. Acht Fixes in drei Klassen (Crash, Hang, Parse).

**Crash-Klasse (#277, P1):**

1. **#277a:** `RegexMutator` castete nach dem bewusst breiten `IsAStringExpression`-Check hart auf `LiteralExpressionSyntax` — interpolierte Patterns (`new Regex($"^{prefix}")`, Level Advanced+) rissen den GESAMTEN Lauf (InvalidCastException, Exit 127, kein Report). Jetzt is-Pattern; interpolierte Patterns sind nicht statisch mutierbar und werden übersprungen.
2. **#277b:** Neue Robustheitsschicht `SafelyMutate` um `mutator.Mutate(...)` im Orchestrator — ein Mutator-Bug überspringt den Mutator für diesen Knoten (WARN mit Mutator-Name, Node-Kind, Issue-Hinweis) statt den Lauf zu töten. Entscheidend: Mutatoren yielden lazy, die Materialisierung passiert IM Guard. Cancellation propagiert bewusst.

**Hang-Klasse (#288 + #297-Epic):**

3. **#288 (G-31):** Der Roslyn-Emit-NRE-Retry lief in einer unbegrenzten while-Schleife; `ScanForCauseOfException` bereinigt nur Solo-Crash-Dateien — bei Interaktions-NREs kam der Tree-Satz unverändert zurück (die äußere MaxAttempt-Schranke zählt Rollback-Runden, nicht NRE-Retries). Jetzt: `HasScanProgress` (Referenzvergleich des Tree-Satzes) + Fünf-Runden-Deckel → diagnostizierbare `CompilationException` statt Hang.
4. **#297a (I-07):** `VsTestRunnerPool.Initialize` lief fire-and-forget — Runner-Konstruktions-Fehler (vstest.console fehlt) waren unobserved, der Pool blieb leer, jeder Konsument wartete ewig. Fehler werden registriert und signalisiert; `RunThis` spiegelt das MTP-Pool-Muster (1-s-Poll, 5-Minuten-Deckel, Fortschritts-Log alle 30 s) und failt fast mit der Konstruktions-Ursache.
5. **#297b (I-07b):** `DiscoveryEventHandler.WaitEnd` nutzte `Monitor.Wait` ohne Timeout — ein vstest-Crash vor DiscoveryComplete hängte die Discovery für immer. Gebundenes Warten (5-Minuten-Default, parametrisierter Overload) mündet in den existierenden Aborted-Pfad des Callers.
6. **#297c (I-15):** `TestingPlatformClient` sammelte bei Disconnect nur Text; registrierte `ResponseListener` wurden nie komplettiert — timeout-lose `WaitCompletionAsync`-Caller (MTP-Discovery, Initial-Runs) warteten ewig auf das null-Changes-Signal eines toten Servers. Disconnect settled jetzt jeden Listener mit einer IOException (Reason/Description); `Complete`/`Fail` nutzen Try-Semantik, damit das Complete/Disconnect-Race harmlos bleibt.

**Parse-Klasse (H-19 + G-37b):**

7. **H-19:** MSBuild-Properties sind User-Input — `GetPropertyOrDefault(bool)` und `GetWarningLevel` nutzen TryParse mit Fallback („yes"/„high" warfen FormatException). `LoadReferencesIterator` überspringt stale Referenzpfade (File.Exists) — die Compilation meldet dann eine präzise Missing-Reference-Diagnose statt einer rohen IOException. Der Resolver fängt die `InvalidOperationException` („no serializer") der Solution-Persistence und wirft eine saubere `InputException`.
8. **G-37b:** `BaselineMutantFilter` nutzt `Enum.TryParse` — Baseline-Reports sind EXTERNER Input (Dashboard, andere Tool-Versionen); ein unbekannter Status lässt den Mutanten Pending (regulärer Test) statt den Lauf mit ArgumentException zu reißen.

**Verifikation:** Build 0/0 · Vollsuite grün · Semgrep 0 auf 10 geänderten Src-Dateien · **Crash-Probe** (interpolierter Regex, `--mutation-level Advanced`, lokale CLI): Exit 0 + WARN statt Exit 127 — Messung im Ship-Gate. **Tag v3.3.7** (Patch).

**Konsequenzen:** Issues #277, #288, #297 geschlossen. G-29/G-30 (Generator-Trees, „get get"-DisplayName) bewusst in #302 belassen. Sprint-183-Vorlage: Config & Nebenläufigkeit (#290/#291/#296/#299/#300).

---

## ADR-057: Config & Nebenläufigkeit — Workspace-Pins, Multi-TFM, Init-Sequenz, Branch-Matching, SSE-Sync (v3.3.8 / Sprint 183)

**Status:** Accepted · **Kontext:** Fünfter Fix-Sprint des Fahrplans (`sprint_178_synthesis.md`). Cluster „Config-Reichweite" + „Races": Optionen, die still nicht wirken, und geteilter Zustand, der unter Nebenläufigkeit korrumpiert.

**Entscheidungen & Fixes (TDD; End-Belege via Live-Proben P-5/P-6 gegen die lokale Release-CLI):**

1. **#290 (H-17) — Konfigurations-Pins erreichen die Roslyn-Workspace:** Die DI-Registrierung konstruiert den `MSBuildWorkspaceProvider` BEVOR die Optionen existieren — `--configuration`/`--target-framework` erreichten nur die parallele MSBuild-Evaluation; die Roslyn-Sicht (SourceFiles, References, **OutputFilePath/TargetDir**) blieb Debug/Erst-TFM. Probe P-5 zeigte Injektion in `bin\Debug` trotz Release-Builds — `-c Release` war faktisch wirkungslos, bei config-spezifischem Code semantisch still falsch. Fix: Self-Factory `IMSBuildWorkspaceProvider.ForProperties` (gibt bei leeren Pins dieselbe Instanz zurück) + der Resolver baut die konfigurierte Workspace EINMAL pro Lauf aus den Lauf-Pins (Configuration, TargetFramework); per-Projekt-Platform-Werte aus Solution-Configs bleiben bei der Parallel-Evaluation. Lebenszyklus gewahrt: Resolver ist transient pro Lauf.
2. **#291 (H-18) — Multi-TFM-Fallback (zwei Schichten):** `RoslynProjectAnalysis.TargetFramework` fiel bei Multi-TFM-Projekten auf die `TargetFrameworks`-LISTE zurück; der Framework-Parser machte daraus `UnsupportedFramework` und der Lauf starb mit dem irreführenden Rat, eine völlig valide csproj zu „fixen" (Probe P-6, Exit 1). Schicht 1: erstes Listen-Segment (`FirstTargetFrameworkFrom`, public + getestet). **Schicht 2 (von der Probe aufgedeckt, Unit-Tests sahen sie nicht):** Die OUTER-Evaluation eines Multi-TFM-Projekts entbehrt JEDER per-Framework-Property — nach Schicht 1 starb der Lauf eine Ebene tiefer an `Language not supported: Undefined`. Der Loader erkennt die Outer-Evaluation (TargetFramework leer, TargetFrameworks gesetzt) und re-evaluiert aufs erste Listen-TFM gepinnt — spiegelt, was die Roslyn-Workspace lädt; evaluierte Property-Daten bleiben nach Dispose der gepinnten Collection gültig.
3. **#296 (I-09) — Initial-Tests sequenziell:** `GetMutationTestInputsAsync` fuhr die Initial-Sessions aller Projekte konkurrent, während die VsTest-Kontextschicht unsynchronisierten Zustand über alle Pool-Runner teilt (plain Dictionaries, Instanz-Ersetzung beim ersten Discovery-Treffer, Cross-Projekt-Clear der Initial-Timings → falsche Timeout-Basis). Sequenzielle Ausführung macht die Race-Pfade strukturell unmöglich (Initial-Läufe dominieren die Gesamtzeit nicht); der Capacity-Hint, der die `VsTests`-Instanz ersetzte (Lost-Update-Pfad), ist entfernt — das Dictionary wird in place befüllt, die Property ist get-only. Red-Test: Overlap-Detektor (2 Projekte, verzögerte Initial-Läufe, Interlocked-Tracking) bewies die Überlappung.
4. **#299 (J-04 + H-27 + J-05) — exaktes Branch-Matching:** `GetTargetCommit` matchte per Substring-`Contains` — `--since-target main` traf auch `maintenance` (erster Enumerations-Treffer gewinnt) → stiller falscher Diff-Base. Fix: exaktes Matching (voller Name oder finale Pfad-Segmente via `/`-Suffix). Der historische Default „master" fällt bei Miss EINMAL laut (WARN) auf „main" zurück statt mit Jargon-Exception zu stranden; die No-Match-Meldung nennt `--since-target`. Beifang J-05: Kurz-SHAs (7–40 hex) werden nachgeschlagen.
5. **#300 (J-06) — Realtime-SSE synchronisiert:** `SseServer._writers` läuft komplett unter Lock (Listener-Task fügt hinzu, Mutant-Threads iterieren — „Collection was modified"-Klasse); `SendEvent` arbeitet auf einem Snapshot und entfernt verlorene Clients unter dem Lock (zusätzlich ObjectDisposed/IO im Catch-Set). Der Listener-Loop behandelt Shutdown-Exceptions als normales Loop-Ende statt als unobserved Fault, der Realtime still beendete. **`CloseSseEndpoint` flusht best-effort pro Writer** — das frühere `Task.WaitAll` warf bei getrennten Clients eine AggregateException, die die BroadcastReporter-Kette brach und nachfolgende Reporter (Json, **Baseline**) am Laufende verschluckte. `RealTimeMutantHandler` nutzt eine `ConcurrentQueue`. Die Race-Konstellationen selbst sind nicht deterministisch provozierbar (im Test dokumentiert); Vertrags-Test: Close mit abgerissenem Client wirft nie.

**Verifikation:** Build 0/0 · Vollsuite grün · Semgrep 0 auf 9 geänderten Src-Dateien · **P-5**: `--configuration Release` → Injektion in `bin\Release\…` · **P-6**: Multi-TFM-ProbeLib läuft mit Exit 0 durch (Baseline Exit 1). **Tag v3.3.8** (Patch).

**Konsequenzen:** Issues #290, #291, #296, #299, #300 geschlossen. Sprint-184-Vorlage: Backlog-Rest (#279-Vollausbau, #280, #287, H-25, #302-Rest).
...
## ADR-059: Äquivalenz-Filter-Korrektheit — Methodengruppen, unsigned-Null-Vergleiche, arithmetische Identitäten (v3.3.10 / Sprint 185)

**Status:** Accepted · **Kontext:** Erster Sprint des externen-360°-Test-Fix-Blocks (185–187). Ein externes Testmanagement-Projekt fand per Black/White-Box gegen v3.3.9 sieben Bugs (`_bug_reporting/BUG_REPORT.md`, Repros `_bug_reporting/testing/`); Sprint 185 räumt den Äquivalenz-Filter-Cluster (EQF-001/003/002, alle in `Mutants/Filters/`, ADR-017-Pipeline). Roadmap der drei Sprints via MAXential+ToT (Variante B, risiko-isoliert). Meta-Befund des Reports bestätigt: die Bugs clustern in älterem v2.x-Filter-Code (3/5 Äquivalenz-Filter fehlerhaft).

**Entscheidungen & Fixes (TDD je Item):**

1. **EQF-001 (High) — `RoslynSemanticDiagnosticsEquivalenceFilter` filtert Methodengruppen fälschlich:** Der immer-aktive Filter verwarf jeden Mutanten, dessen `ReplacementNode` eine bloße Methodengruppe ist (StringMethodMutator `s.EndsWith`, LinqMutator `xs.Max` — die Invocation ist der Parent, nicht Teil des Replacements), als „equivalent". `GetSpeculativeSymbolInfo(BindAsExpression)` liefert für eine Methodengruppe `Symbol==null` + `CandidateReason==OverloadResolutionFailure`, aber NICHT-leeres `CandidateSymbols` — die bestehende Bedingung `Symbol is null && CandidateReason != None` war erfüllt → killbare Defaults-Mutanten still entfernt, Score aufgebläht. Fix (MAXential+ToT: A1 CandidateSymbols-Gate 0.93 vs. A3 syntaktischer Guard 0.71 vs. A2 Invocation-Binding 0.38): zusätzliches Konjunktiv `&& info.CandidateSymbols.IsDefaultOrEmpty` — eine Methodengruppe hat auflösbare Kandidaten und ist valide, nicht ungültig. Symmetrisch auf den Statement-Pfad angewandt. Der Filter ist eine Compile-Runden-Optimierung, kein Korrektheits-Gate (echte CompileErrors fängt der RollbackProcess). **E2E-Probe (bug01, lokale CLI): 8 Methodengruppen-Mutanten — 6 StringMethod-Paare + Linq `Min→Max` — werden jetzt GETESTET (Score 75 %, 0 als equivalent ge-Ignored) statt wie in v3.3.9 gefiltert; die Linq-Extension-Methodengruppe `xs.Max` Killed = Blast-Radius end-to-end belegt** (Unit-Harness deckt den import-abhängigen Linq-Fall nicht ab — „Live-Probe > Unit-Surface").

2. **EQF-003 (Med) — `ConservativeDefaultsEqualityFilter` verwirft killbare unsigned-Null-Vergleiche:** Der Filter flaggte JEDE `==`/`!=`-zu-geordnet-Mutation eines unsigned-Operanden gegen Literal 0 pauschal als äquivalent. Von acht Kombinationen sind nur zwei echt äquivalent. ÜBER DEN REPORT HINAUS entdeckt: die Äquivalenz hängt von der OPERAND-POSITION ab (`u <= 0` ⟺ `u == 0`, aber `0 <= u` ist immer wahr). Fix: positions-normalisiert (Literal links ⇒ Vergleichsrichtung via `FlipComparison` spiegeln), dann nur `Equals→LessThanOrEqual` und `NotEquals→GreaterThan` flaggen; die übrigen sechs sind killbar. Bug-pinnender Test (`x==0→x<0` als äquivalent) umgeschrieben; Wahrheitstabelle 12 Fälle (beide Positionen).

3. **EQF-002 (Low) — zwei inerte Filter, ehrlich getrennt:** (a) **`IdentityArithmeticFilter`** (realer Bug): Der `originalBinary.IsEquivalentTo(replacementBinary)`-Guard ist syntaktisch (nur bei No-op wahr) → echte Identitäts-Äquivalente (`x+0→x-0`, `x*1→x/1`, beide `== x`) überlebten als Survived und senkten den Score künstlich. Fix struktur-basiert (right-identity): gleicher linker Operand, Identitäts-Literal als RECHTER Operand beider, Operatoren aus derselben Gruppe (add/subtract für 0, multiply/divide für 1). ÜBER DEN REPORT HINAUS: die linke-Literal-Formen (`0-x` = Negation, `1/x` = Reziprok) sind KEINE Identitäten und bleiben killbar (ein naiver Report-Fix „one literal-0 operand" hätte hier einen neuen Bug eingeführt). Bug-pinnender Test umgeschrieben; Wahrheitstabelle 10 Fälle. (b) **`IdempotentBooleanFilter`** (konzeptuell inert): MAXential+ToT-Entscheidung (belassen+dokumentieren 0.92 vs. entfernen 0.43 vs. spekulativ-fixen 0.31). Anders als IdentityArithmetic existiert hier KEIN realer äquivalenter Mutant — jede Operator-/Literal-Mutation eines `x&&true`/`x||false`-Patterns ändert den Wert oder zerstört das Pattern, und kein Mutator emittiert `!!x→x`. Ein spekulativer Struktur-Fix wäre unfalsifizierbar (TDD-widrig); der Double-Negation-Pfad ist korrekt (nur heute ungenutzt). Entscheidung: Code unverändert, präziser Inertheit-Kommentar ergänzt. Inert ≠ false-survivor, solange keine Äquivalente existieren → kein Schaden.

**Verifikation:** Build 0/0 (TWAE) · Stryker.Core.Tests 567/567 grün (ein bekannter Log-Capture-Flake im Parallel-Lauf, isoliert grün + Doppellauf grün) · Semgrep 0 auf den geänderten Filter-Dateien · 57/57 Filter-Tests · E2E-Probe bug01 (8 getestet, 0 ge-Ignored, Score 75 %). Drei bug-pinnende Tests, die das defekte Verhalten festschrieben, korrigiert. **Tag v3.3.10** (Patch — Bugfixes ohne API-Änderung).

**Konsequenzen:** EQF-001/003/002 vollständig adressiert (drei der „3/5 kaputten Filter" des Reports behoben, der vierte ehrlich als inert dokumentiert). Folge-Sprints des Blocks: 186 Quick-Wins (MAT-001/SOL-001/RUN-001 → v3.3.11), 187 INJ-001 SOLO (→ v3.3.12). Methodik-Bestätigung aus Gegenrichtung: externe Black/White-Box-Sicht + Live-Proben deckten Filter-Bugs auf, die das interne 139-Findings-Programm (173–184) nicht sah.

---

## ADR-060: Quick-Wins — MathMutator-Semantic-Model, --solution-cwd-Unabhängigkeit, README-Kompatibilität (v3.3.11 / Sprint 186)

**Status:** Accepted · **Kontext:** Zweiter Sprint des externen-360°-Test-Fix-Blocks (185–187). Drei niedrig-Risiko-Quick-Wins aus unterschiedlichen Subsystemen (Mutator / CLI-Config / Doku); gemeinsame Klammer = kleine, unabhängige Korrekturen (Variante B der Roadmap, MAXential+ToT).

**Entscheidungen & Fixes (TDD wo Code):**

1. **MAT-001 (Med) — `MathMutator` mutiert `Math.X(...)`-Member-Aufrufe nie (mit Semantic-Model):** Der Member-Call-Pfad prüfte `symbol.ContainingType` des Receivers `Math`, aber `Math` bindet auf den Top-Level-Typ `System.Math`, dessen `ContainingType` `null` ist → `null == "System.Math"` false → `yield break`. Asymmetrie: Der DirectCall-Pfad (`using static System.Math`) prüft das METHODEN-Symbol, dessen `ContainingType` `System.Math` ist, und funktionierte → der Bug blieb verborgen. Fix: im Member-Pfad das Receiver-SYMBOL selbst vergleichen (`symbol?.ToString() == "System.Math"`). Die bestehenden Unit-Tests fuhren den `semanticModel == null`-Text-Fallback und maskierten den Bug — neuer Red-Test mit ECHTEM Semantic-Model. **E2E-Probe (bug03): `Math.Ceiling(x)` (Member-Call) erzeugt jetzt einen Mutanten (Ceiling→Floor, Killed) — vorher 0; 2 Math-Mutanten statt 1.**

2. **SOL-001 (Low) — `--solution` bei cwd-Mismatch still ignoriert:** `IsSolutionContext` verlangte zusätzlich zum gesetzten `SolutionPath`, dass das Working-Directory das Solution-Verzeichnis IST → ein expliziter `--solution`-Pfad (auch absolut) aus einem anderen cwd wurde verworfen (Single-Project-Fallback + irreführende „more than one project reference"-Meldung, die zum bereits gesetzten Flag riet). Fix (MAXential+ToT: A cwd-Konjunkt entfernen 0.93 vs. C Hybrid 0.61 vs. B Warnung-Workaround 0.4): `IsSolutionContext => SolutionPath != null`. Sicher, weil `SolutionPath` ausschließlich via `SolutionInput.Validate` (= `--solution`) gesetzt wird → `!= null` ist exakt „explizit opt-in". Standard-CLI-Semantik (Pfad-Flag wirkt von überall); `ScanInSolutionMode` blieb unverändert (funktionierte korrekt — nur die Aktivierung war falsch gated).

3. **RUN-001 (Med) — README über-behauptet TUnit- + MTP-Support:** Die Kompatibilitäts-Tabelle listete TUnit (Framework) + Microsoft Testing Platform (Runner) als unterstützt; tatsächlich lehnt Stryker MTP ab und TUnit (MTP-exklusiv) findet 0 Tests. Doku-Korrektur: Test-Runner = nur VsTest; Test-Frameworks = xUnit/MSTest/NUnit; neue Anmerkung markiert MTP + TUnit als roadmapped (Verweis stryker-net#3094 + ADR-044) und nennt die .NET-10-SDK-VsTest-Target-Entfernung.

**Verifikation:** Build 0/0 (TWAE) · Stryker.Core.Tests 572/572 grün · Semgrep 0 auf den geänderten Dateien · E2E-Probe bug03 (2 Math-Mutanten, Member-Call gefixt). Zwei Red-Tests aus echtem Semantic-Model/cwd-Mismatch. **Tag v3.3.11** (Patch — Bugfixes ohne API-Änderung).

**Konsequenzen:** MAT-001/SOL-001/RUN-001 adressiert — 6 der 7 externen Bugs nun behoben. Verbleibend: Sprint 187 INJ-001 SOLO (declaration-level-Injektion, architektur-schwer → v3.3.12). Methodik: MAT-001 bestätigt erneut „Live-Probe/Semantic-Model > Null-Model-Unit-Surface" (die bestehenden Tests maskierten den Bug, weil sie den Fallback-Pfad fuhren).

---

## ADR-061: INJ-001 — Declaration-level-Injektion: MethodBodyReplacement gefixt, Constraint-Mutatoren als nicht-injizierbar erkannt (v3.3.12 / Sprint 187)

**Status:** Teil A Accepted; **Teil B superseded von ADR-062** (Sprint 188 — Constraint-Mutatoren entfernt statt vertagt) · **Kontext:** Dritter und LETZTER Sprint des externen-360°-Test-Fix-Blocks (185–187). INJ-001 (Report): `MethodBodyReplacementMutator` + `GenericConstraintMutator` (beide `MethodDeclarationSyntax`) lassen sich nicht injizieren → still zu CompileError; 2 `All`-Operatoren funktionslos. Architektur-Analyse via MAXential+ToT + Live-Probe (bug02). Backed by: Inject-Mechanik (RoslynHelper.TryInjectMutation prüft `sourceNode.Contains(OriginalNode)`; höchster Frame = Method-BODY via BaseFunctionOrchestrator; Inject-Overloads nur Expression/Statement/Block).

**Architektur-Befund (empirisch, bug02 All/Complete, lokale CLI) — INJ-001 zerfällt in zwei verschieden-schwere Teile:**

1. **Teil A — `MethodBodyReplacementMutator` (GEFIXT):** ändert nur den BODY. Alt: OriginalNode = MethodDeclaration (Member-Level) → in keinem Inject-Frame enthalten → Soft-Fail „sourceNode is null". **Die Live-Probe widerlegte auch den naiven Fix** (OriginalNode = node.Body allein scheiterte weiter — der BaseFunctionOrchestrator injiziert left-over-Mutationen nur über den Expression-Body-Pfad `GetBodies(sourceNode).expression`, null für Block-Body). Lösung: Mutator zu `TypeAwareMutatorBase<BlockSyntax>` umbauen (zielt auf den direkten Method-Body-Block eines nicht-async-Methods, Return-Typ vom Parent) — analog dem funktionierenden `BlockMutator`; so wird der Body-Block am Block-Level injiziert. `Type = Mutator.Statement` (NICHT `Block`), damit der `IgnoreBlockMutantFilter` (verwirft `Mutator.Block`-Mutanten, deren Span aktive innere Mutanten überdeckt) die Body-REPLACEMENT-Mutation nicht fälschlich schluckt. **E2E bug02: 'Echo' vorher CompileError-Soft-Fail → jetzt KILLED** (injiziert + getestet). Der Operator ist funktional.

2. **Teil B — `GenericConstraintMutator` + `GenericConstraintLoosenMutator` (VERTAGT, User-Entscheidung):** ändern SIGNATUR-Constraints (`where T : …`). Befund: (a) `ConstraintClause`/`TypeParameterConstraint` kommen im gesamten Orchestrator NICHT vor → Constraint-Klauseln werden nie besucht; (b) Constraint-Mutationen sind Compile-Zeit-Metadaten, im Stryker-Modell (ein Assembly, Laufzeit-`MutantControl.IsActive`-Schaltung) prinzipiell nicht repräsentierbar (eine `where`-Klausel kann nicht zur Laufzeit umgeschaltet werden). **Das Report/Roadmap-„Vorbild" (GenericConstraintLoosen injiziert korrekt als clause-level-Beleg) ist empirisch FALSIFIZIERT** — Loosen feuert nie (0 Mutanten für `where T : class` bei All/Complete); GenericConstraint emittiert ausschließlich Soft-Fails. Beide sind funktionslos by design; ein Orchestrator-Umbau löst das fundamentale „nicht zur Laufzeit schaltbar" nicht (Scheinlösung). MAXential+ToT-Empfehlung B2 (deaktivieren, ToT 0.88) vs. B1 entfernen (0.55) / B4 NotInjectable-Status (0.5) / B3 Umbau (0.25). **User-Entscheidung: VERTAGEN ins #279-Epic** (nicht deaktivieren, Status bleibt wie heute), dokumentiert als bekannt-nicht-injizierbar (XML-`<remarks>` an beiden Klassen + dieser ADR; die alten Doc-Behauptungen über CompileError-Klassifikation korrigiert).

**Verifikation:** Build 0/0 (TWAE) · Stryker.Core.Tests 574/574 · Dogfood-Orchestrierung 36/37 (1 skip) · Semgrep 0 · E2E bug02 (MethodBodyReplacement KILLED). **Tag v3.3.12** (Patch).

**Konsequenzen:** MethodBodyReplacement ist als `All`-Operator funktional (INJ-001 Teil A behoben). Teil B (2 Constraint-Mutatoren) bleibt offen im #279-Epic, ehrlich dokumentiert als architektonisch-nicht-injizierbar (keine Scheinlösung erzwungen). **Externer-360°-Test-Fix-Block (185–187) abgeschlossen: 6 der 7 Bugs vollständig behoben (EQF-001/003/002, MAT-001, SOL-001, RUN-001) + INJ-001 Teil A; INJ-001 Teil B als Modell-Limit dokumentiert.** Lehre (3-fach in EINEM Sprint): die Live-Probe widerlegte den naiven Fix (Block-Retarget), ein Roadmap-„Vorbild" (Loosen) UND deckte den Filter-Folgeeffekt (IgnoreBlockMutantFilter) auf — reine Analyse hätte alle drei durchgehen lassen.

---

## ADR-062: INJ-001 Teil B — Constraint-Mutatoren entfernt (semantisches Modell-Limit) (v3.3.13 / Sprint 188)

**Status:** Accepted · **Supersedes:** ADR-061 Teil B (dort ins #279-Epic vertagt) · **Kontext:** Der User wies an, das in Sprint 187 vertagte INJ-001 Teil B endgültig zu lösen. Vertiefte Architektur-Analyse via MAXential (18 Gedanken, 2 Branches: rebuild/reframe — beide verworfen) + ToT (best_path „entfernen" 0.92) + IL-Live-Probe.

**Kern-These (adversarial geprüft, kein Gegenbeispiel):** Generic Constraints (`where T : …`) sind REIN COMPILE-TIME und erzeugen keinen IL. Eine Constraint-Mutation kann daher prinzipiell nur zwei Ausgänge haben — **CompileError** (wenn der Body die Constraint-Fähigkeit nutzt, z.B. `t.CompareTo` bei `where T:IComparable`) oder einen **äquivalenten, unkillbaren Survivor** (wenn nicht genutzt). Ein killbarer Laufzeit-Mutant ist UNMÖGLICH. Auch Overload-Resolution liefert kein Gegenbeispiel (zwei Methoden, die sich nur im Constraint unterscheiden, sind in C# verboten — CS0111).

**IL-Live-Probe (Beleg, Projektkultur „Live-Probe > Analyse"):** Zwei generische Methoden, identisch bis auf `where T:class`, via Reflection verglichen → `GetMethodBody().GetILAsByteArray()` **byte-identisch** (4 == 4 Bytes); der Constraint lebt ausschließlich in der Typparameter-Metadata (`ReferenceTypeConstraint` vs `None`). Empirischer Beweis: ein kompilierender Constraint-Drop hat keinen IL-Unterschied → keinen Laufzeit-Unterschied → ist zwingend äquivalent.

**Architektur-Befund (gegen aktuellen main):** `MutantPlacer` hat genau zwei Inject-Pfade (`IfEngine.InjectIf`/Statement, ternary/Expression) — kein Member-/Declaration-Pfad; die `Mutator`-Enum hat keinen Constraint-Typ (beide Mutatoren missbrauchten `Mutator.Statement`). `GenericConstraintMutator` (OriginalNode = MethodDeclaration) emittierte nur CompileError-Soft-Fails; `GenericConstraintLoosenMutator` (OriginalNode = TypeParameterConstraintClauseSyntax) feuerte 0 Mutanten (Orchestrator besucht Constraint-Klauseln nie).

**Entscheidung — beide Mutatoren ENTFERNT (O-A):** Dies REVIDIERT die Sprint-187-Bewertung (ADR-061: entfernen 0.55 < deaktivieren 0.88), weil die vertiefte Analyse zwei zuvor unterbewertete Fakten festigte: (1) Constraint-Mutationen können per se keinen killbaren Mutanten erzeugen (IL-Beleg) → „injizierbar machen" via Per-Mutant-Rebuild ist ein Kategorienfehler, der zudem Strykers Performance-USP (ADR-021) bricht und den Score durch falsche Survivor verfälscht (strikt dominiert, ToT 0.10); (2) die Mutatoren sind konzeptuell REDUNDANT — die einzige beobachtbare Constraint-Wirkung (welche Body-Operationen erlaubt sind) ist bereits voll mutationsabgedeckt (Relational-/Equality-/Method-/Statement-Mutatoren), Entfernen verliert keine Coverage. Deaktivieren (O-B 0.65) hinterließe tote, „reparatur"-einladende Code-Leichen. Entfernen ist die ehrlichste, wartungsärmste Variante (Projektkultur „ehrlich statt kosmetisch", vgl. IdempotentBoolean-Doku ADR-059).

**Umsetzung:** `GenericConstraintMutator.cs` + `GenericConstraintLoosenMutator.cs` + beide Test-Dateien gelöscht (git rm); 2 Registrierungen aus `CsharpMutantOrchestrator` entfernt; 2 `<see cref="GenericConstraintMutator"/>` in fremden Mutatoren (`AsSpanAsMemoryMutator`, `TaskWhenAllToWhenAnyMutator`) + 1 Prosa-Kommentar (`RoslynHelper`) bereinigt (sonst CS1574 unter TWAE). Mutator-Katalog **52 → 50** (Defaults 26, Stronger 18→17, All-only 8→7).

**Verifikation:** Build 0/0 (TWAE) · Stryker.Core.Tests 562/562 (+ bekannter Flaky CommentParser-Log-Race isoliert grün; 11 entfernte Constraint-Tests erklären 574→563) · Dogfood-Orchestrierung 1239/1248 (9 skip, 0 Fehler — keine erwartete Mutanten-Zählung ändert sich, da die Mutatoren nie erfolgreiche Mutanten beitrugen) · IL-Live-Probe · Semgrep 0. **Tag v3.3.13** (Patch).

**Konsequenzen:** INJ-001 ist ENDGÜLTIG geschlossen (Teil A gefixt Sprint 187, Teil B entfernt Sprint 188); das #279-Epic ist von der INJ-001-Constraint-Altlast befreit. `MutatorReflectionProperties` (Reflection-Discovery) bleibt grün (vollständig dynamisch, keine hartcodierte Zahl). Lehre: Ein „nicht injizierbarer Mutator" ist nicht automatisch ein zu-fixender Bug — wenn das Mutationsziel semantisch keinen killbaren Mutanten erzeugen KANN, ist Entfernen der korrekte Fix, nicht Engineering. „Fixen" hieß hier: ein konzeptuell ungeeignetes Feature ehrlich bereinigen.
...
|---------|-------|-------|----------|
| 0.45.0 | 2026-06-15 | Claude Opus 4.8 (1M context) (Co-Authored mit pgm1980) | Sprint 188 v3.3.13: **ADR-062** (supersedes ADR-061 Teil B) — INJ-001 Teil B endgültig gelöst: `GenericConstraintMutator` + `GenericConstraintLoosenMutator` ENTFERNT (statt Sprint-187-Vertagung ins #279-Epic). Vertiefte Analyse MAXential (18 Gedanken, Branches rebuild/reframe verworfen) + ToT (entfernen 0.92) + IL-Live-Probe: Generic Constraints sind rein Compile-Time, Method-Body-IL byte-identisch mit/ohne Constraint (Reflection-Beleg) → eine Constraint-Mutation erzeugt NIE einen killbaren Mutanten (nur CompileError oder äquivalenter Survivor), zudem redundant zu Body-Mutatoren (Relational/Equality/Method/Statement). „Injizierbar machen" (Per-Mutant-Rebuild) wäre Kategorienfehler (bricht ADR-021-Performance-USP, verfälscht Score; ToT 0.10). 2 Mutatoren + 2 Test-Dateien gelöscht (git rm), 2 Fremd-`<see cref>` (AsSpanAsMemory/TaskWhenAllToWhenAny) + 1 RoslynHelper-Kommentar bereinigt (CS1574); Katalog 52→50 (26+17+7). Build 0/0, Core 562/562 (+Flaky isoliert grün), Dogfood 1239/1248, Semgrep 0. INJ-001 endgültig geschlossen; #279-Epic von Constraint-Altlast befreit. |
| 0.44.0 | 2026-06-15 | Claude Opus 4.8 (1M context) (Co-Authored mit pgm1980) | Sprint 187 v3.3.12: **ADR-061** — INJ-001 (externer 360°-Test, Fix-Block 185–187 Sprint 3/3, ABSCHLUSS). Architektur-Analyse via MAXential+ToT + Live-Probe (bug02). Teil A GEFIXT: `MethodBodyReplacementMutator` zu `TypeAwareMutatorBase<BlockSyntax>` umgebaut (zielt direkten Method-Body-Block, `Type=Mutator.Statement` gegen IgnoreBlockMutantFilter); E2E bug02 'Echo' CompileError-Soft-Fail → KILLED. Live-Probe widerlegte den naiven Block-Retarget (BaseFunctionOrchestrator injiziert left-over nur via Expression-Body-Pfad). Teil B VERTAGT (User-Entscheidung, #279-Epic): `GenericConstraintMutator` + `GenericConstraintLoosenMutator` mutieren Signatur-Constraints, im Laufzeit-Schalt-Modell prinzipiell nicht injizierbar (Orchestrator besucht ConstraintClauses nie; nicht per MutantControl schaltbar) — Roadmap-„Vorbild" Loosen empirisch falsifiziert (0 Mutanten); als nicht-injizierbar dokumentiert (XML-remarks), nicht deaktiviert. Build 0/0, Core 574/574, Dogfood 36/37, Semgrep 0. |
| 0.43.0 | 2026-06-15 | Claude Opus 4.8 (1M context) (Co-Authored mit pgm1980) | Sprint 186 v3.3.11: **ADR-060** — Quick-Wins (externer 360°-Test, Fix-Block 185–187 Sprint 2/3). MAT-001 (Med): `MathMutator` Member-Pfad prüfte `ContainingType` des Receiver-Typsymbols `System.Math` (= null) statt des Symbols selbst → `Math.X(...)` nie mutiert mit Semantic-Model; Fix `symbol?.ToString()`. Bestehende Unit-Tests fuhren Null-Model-Fallback (maskierte den Bug) → neuer Red mit echtem Model; E2E bug03: 2 Math-Mutanten (Member-Call gefixt). SOL-001 (Low): `IsSolutionContext` an cwd==SolutionDir gebunden → `--solution` aus fremdem cwd ignoriert; Fix `=> SolutionPath != null` (MAXential+ToT A 0.93; SolutionPath nur via --solution gesetzt). RUN-001 (Med): README-Kompat-Tabelle korrigiert (MTP+TUnit als roadmapped, #3094/ADR-044). Build 0/0, Core 572/572, Semgrep 0. |
| 0.42.0 | 2026-06-15 | Claude Opus 4.8 (1M context) (Co-Authored mit pgm1980) | Sprint 185 v3.3.10: **ADR-059** — Äquivalenz-Filter-Korrektheit (externer 360°-Test, Fix-Block 185–187 Sprint 1/3). EQF-001 (High): `RoslynSemanticDiagnosticsEquivalenceFilter` filterte Methodengruppen (StringMethod immer, Linq bei datei-level using) fälschlich als equivalent — Fix `CandidateSymbols.IsDefaultOrEmpty`-Gate (MAXential+ToT A1 0.93), beide Pfade; E2E bug01: 8 Mutanten getestet statt ge-Ignored, Linq `xs.Max` Killed. EQF-003 (Med): `ConservativeDefaultsEqualityFilter` — nur 2/8 unsigned-Kombis äquiv, positions-normalisiert (`FlipComparison`; über Report hinaus). EQF-002 (Low): `IdentityArithmeticFilter` struktur-basierte right-identity (`0-x`/`1/x`-Fallen ausgeschlossen, über Report hinaus); `IdempotentBooleanFilter` ehrlich als konzeptuell inert dokumentiert (MAXential+ToT 0.92, kein spekulativer Fix). 3 bug-pinnende Tests umgeschrieben. Build 0/0, Core 567/567, Semgrep 0, E2E-Probe grün. |
| 0.41.0 | 2026-06-12 | Claude Fable 5 (Co-Authored mit pgm1980) | Sprint 184 v3.3.9: **ADR-058** — Backlog-Rest, LETZTER Fahrplan-Sprint (Programm 173–184 komplett). #287/G-22: RegisterCoverage HashSet-Drop-in (Benchmark 11,2× bei 10k Mutanten), GetCoverageData materialisiert einmal (Reflection-Vertrag), G-24-Handler raus. #280/F-01: Mutator.Number additiv, Konstanten-Mutatoren umkategorisiert (ignore-mutations['linq'] deaktivierte sie still), Type-Pins ergänzt. #279-Batch-1 nach Hausblaupause (Abstain-bei-Unbekannt): UOI-Typ/Symbol-Gate inkl. Nicht-Wert-Symbole (Namespace/Methodengruppe — von der CE-Probe aufgedeckt), ROR-Ordnung nur auf geordneten Operanden, ConstructorNull-IsReferenceType, TypeDrivenReturn-Async-Guard, Doc-F-30 (6 Dateien); Epic offen für Block/AsSpanAsMemory/F-08. H-25: Threshold-Quervalidierung gegen Effektivwerte. I-11: MTP-Singleton-Guard statt stillem mutantId=-1 (2 Bug-pinnende Upstream-Tests umgeschrieben). **CE-Probe 56 %→29,4 %**; Rest = bewusst offene Epic-Klassen. |
| 0.40.0 | 2026-06-12 | Claude Fable 5 (Co-Authored mit pgm1980) | Sprint 183 v3.3.8: **ADR-057** — Config & Nebenläufigkeit (#290/#291/#296/#299/#300; Fahrplan-Sprint 5/6). #290: IMSBuildWorkspaceProvider.ForProperties-Self-Factory + Resolver baut konfigurierte Workspace einmal pro Lauf — --configuration/--target-framework erreichen jetzt die Roslyn-Sicht (P-5: Injektion in bin\Release statt bin\Debug). #291: Multi-TFM-Fallback parst erstes Listen-Segment statt InputException auf der Rohliste (P-6: Exit 0). #296: Initial-Tests sequenzialisiert (Overlap-Detektor-Red-Test) + VsTests-Lost-Update-Pfad entfernt (get-only, in-place-Befüllung). #299: exaktes Branch-Segment-Matching statt Substring-Contains (main ≠ maintenance), master→main-Fallback mit WARN, --since-target-Meldung, Kurz-SHA-Lookup (J-05). #300: SseServer-Writer unter Lock + Snapshot-Send, Listener-Shutdown als normales Loop-Ende, CloseSseEndpoint best-effort (AggregateException brach die Reporter-Kette → Json-/Baseline-Verlust), ConcurrentQueue im Handler. |
| 0.39.0 | 2026-06-12 | Claude Fable 5 (Co-Authored mit pgm1980) | Sprint 182 v3.3.7: **ADR-056** — Robustheit (#277a/b, #288, #297a/b/c, H-19, G-37b; Fahrplan-Sprint 4/6). Crash-Klasse: RegexMutator-is-Pattern statt Cast (interpolierte Patterns überspringen) + SafelyMutate-Guard um mutator.Mutate (Lazy-Materialisierung IM try, OCE propagiert). Hang-Klasse: HasScanProgress + 5-Runden-Deckel im Roslyn-NRE-Retry → CompilationException; VsTest-Pool beobachtet Runner-Bau-Fehler + RunThis nach MTP-Muster (Fail-fast mit Ursache); WaitEnd mit 5-Min-Timeout in den Aborted-Pfad; MTP-Disconnect settled alle ResponseListener (IOException, Try-Semantik). Parse-Klasse: TryParse-Guards für MSBuild-Properties, File.Exists vor CreateFromFile, InputException für unlesbare Solutions, Enum.TryParse für Fremd-Baselines. Crash-Probe: interpolierter Regex + Advanced → Exit 0 + WARN statt Exit 127. |
| 0.38.0 | 2026-06-12 | Claude Fable 5 (Co-Authored mit pgm1980) | Sprint 181 v3.3.6: **ADR-055** — Score-Integrität (#286/G-15, G-17, G-19, J-01; Fahrplan-Sprint 3/6). G-17: NoOpMutationFilter als Filter #0 (AreEquivalent trivia-insensitiv — False-Survivor-Klasse der No-op-Emissionen zentral gefangen). G-19: equivalence-gefilterte Mutationen als Ignored mit „Equivalent mutant (filter: <Id>)" registriert statt spurlosem continue (nie injiziert; Red per Stash-Roundtrip bewiesen). #286/G-15: ADR-032-Drop-Pfade melden verworfene Subtrees via onMutationsDropped-Callback → FlagDroppedMutants setzt Pending-Geister auf CompileError (ID-Extraktion MutantPlacer.ExtractMutantIds; 4 Call-Sites; generischer Geister-Detektor in den Slot-Validation-Integrationstests). J-01: Pending-Reste werden am Session-Drain als Ignored finalisiert — kein schema-fremdes „Pending" mehr im Final-Report, Realtime-Reporting erreicht sie genau einmal. |
| 0.37.0 | 2026-06-12 | Claude Fable 5 (Co-Authored mit pgm1980) | Sprint 180 v3.3.5: **ADR-054** — CE-Noise I (#278/#284/#285, Fahrplan-Sprint 2/6). #278: IsPatternExpressionMutator skippt designation-bindende Patterns (CS0165-Klasse; Discards bleiben mutierbar). #284a: PatternSyntaxOrchestrator + is-Pattern-Kontext-Check — Pattern-Mutationen in designation-tragenden is-Patterns zur Store-Zeit auf Block-Level (CS0128-Klasse; Switch-Arme/Case-Labels bewusst unverändert). #284b: ContainsDeclarations + SingleVariableDesignation (Recursive-/Var-/List-Pattern-Designationen; Lambda-scoped bleibt ausgeschlossen); neue Helfer ContainsBindingDesignation/IsInsideVariableBindingIsPattern. #285a: MutantPlacer.CanHostValueMarker — target-typed nie wrappen (CS8754-Klasse statt Rollback-Heilrunde mutant −1), konstant+mutationsfrei nie (CS0266-Klasse), User-Code-ausführende Initializer immer (Static-Tracking-Pin für `= Compute()`). #285b: Property-Initializer-Pfad auf MustInjectCoverageLogic gegated. Manifest-Pin 3.3.4. Erster Sprint der Serena-Ära (Symbol-Navigation, Memory vor/nach Sprint). |
| 0.36.0 | 2026-06-11 | Claude Fable 5 (Co-Authored mit pgm1980) | Sprint 179 v3.3.4: **ADR-053** — Quick-Wins + P1 aus dem 360°-Fahrplan (sprint_178_synthesis.md). 9 Fixes TDD-verifiziert: #282 P1 (out-Lambda-NRE → default-Literal, Probe Exit 127→0), #294 (Contains-Inversion + erste Mengen-API-Tests), #295 (MsTest-Flag `|=`), #283 (case-Label/goto-case-Fence, Probe-CE-Klasse eliminiert), #292 (Enrichment-Guards), #273 (deterministische Test-Bounds), #274 (Config-Key raus → Nightly-11/11-Ziel), H-05/J-11 (#302). Dazu G-01-MECHANIK-KORREKTUR: needReturn-Pfad leistet Ending-Return bereits — No-op-Fix durch Red-Verifikation vor Ship entdeckt+verworfen, Pin-Test stattdessen; Proben-Block-CEs als G-30-Rollback-Kollateral re-attribuiert (echte Ursache #284/#278). |
