...
2. **#291 (H-18) — Multi-TFM-Fallback:** `RoslynProjectAnalysis.TargetFramework` fiel bei Multi-TFM-Projekten auf die `TargetFrameworks`-LISTE zurück; der Framework-Parser machte daraus `UnsupportedFramework` und der Lauf starb mit dem irreführenden Rat, eine völlig valide csproj zu „fixen" (Probe P-6, Exit 1). Fix: erstes Listen-Segment (`FirstTargetFrameworkFrom`, public + getestet) — exakt das, was die Roslyn-Workspace ohnehin lädt.
