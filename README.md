...
- **Cold-run wall-clock variance** (v3.3.0 doc-note). The very first mutation run on a fresh machine pays JIT-compilation + NuGet symbol-load amortization in the initial test phase — typically 4-5× slower than warm runs. Subsequent runs stabilize at the warm timing. CI runners starting cold every time will see this on every run; consider pre-warming the tool with a no-op `--version` call earlier in the same job.
- **Per-mutant coverage attribution is class-fixture-bound** (v3.3.0 doc-note, see [ADR-048](_docs/architecture%20spec/architecture_specification.md)). When a test constructs its own SUT instance inside the test body (e.g., to inject a `Mock<ILogger<T>>` per-test for assertion of log emissions), `--coverage-analysis perTest` may fail to attribute mutations on that SUT's methods to the test. The recommended pattern is to wire the `Mock<ILogger>` at the class-fixture level so every test reuses the same SUT instance. SUT-instance-aware coverage capture is roadmapped for v3.4+.
- **`dotnet-tools.json` `rollForward` pinning** (general advice for bug filers). If your `dotnet-tools.json` pins a specific Stryker version with `"rollForward": false`, the in-tool upgrade banner does **not** roll your local install forward — only the announcement. Before filing a bug, run `dotnet tool update --local dotnet-stryker-netx --version <latest>` (or edit `dotnet-tools.json` and `dotnet tool restore`) and re-verify against the latest released version.
