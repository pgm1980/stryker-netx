...
function Run-Stryker {
  ...
  Write-Info "Running dotnet-stryker-netx in '$WorkingDirectory' with args: $argumentText"
  ...
  if ($LASTEXITCODE -ne 0) {
    if ($ContinueOnError) {
      Write-Warn "dotnet-stryker-netx failed in $WorkingDirectory with exit code $LASTEXITCODE (continuing)"
    } else {
      throw "dotnet-stryker-netx failed in $WorkingDirectory with exit code $LASTEXITCODE"
    }
  }
}
...
$stryker = Join-Path $toolPath "dotnet-stryker-netx"
