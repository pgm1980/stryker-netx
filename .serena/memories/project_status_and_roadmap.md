...
| 183 | Config & Nebenläufigkeit | **IN PROGRESS (Branch feature/183-config-concurrency, gestartet 2026-06-12)**: #290/H-17 (DI registriert MSBuildWorkspaceProvider mit Default-Ctor — ServiceCollectionExtensions.cs:47; Factory-Lambda muss Configuration/Platform/TargetFramework aus Optionen in den existierenden Ctor-Overload füttern; P-5-Probe: -c Release injizierte in bin\\Debug), #291/H-18 (RoslynProjectAnalysis.TargetFramework:251 fällt auf TargetFrameworks-LISTE zurück → GetNuGetFramework → UnsupportedFramework-InputException; erstes Semikolon-Segment + Meldung mit --target-framework-Hinweis; P-6-Probe Multi-TFM Exit 1), #296/I-09 (GetMutationTestInputsAsync Task.WhenAll → sequenzialisieren + VsTests-Instanz-Ersetzung Zeile 224 + ClearInitialResult wischt fremde Projekte), #299/J-04 (GitInfoProvider.GetTargetCommit:87 Substring-Contains → exaktes Segment-Matching, FriendlyName == target bzw. Canonical endet auf /<target>; master-Default-Fallback; --since-target-Fehlermeldung), #300/J-06 (SseServer._writers plain List + _delayedEventQueue plain Queue → Sync; Listener-Loop try/catch; CloseSseEndpoint best-effort-Flush pro Writer — AggregateException brach Broadcast-Kette → Json/Baseline-Verlust). Erfolgsmaß: P-5 (-c Release → bin\\Release-Injektion), P-6 (Multi-TFM läuft durch) |
