# Status & Roadmap (Stand 2026-06-15, nach Sprint 188 / v3.3.13)
...
## 🆕 Externer-360°-Test-Fix-Block (Sprints 185–188) — ✅ ABGESCHLOSSEN (Stand 2026-06-15)
...
- **Sprint 188 (✅ v3.3.13 shipped 2026-06-15)** — INJ-001 Teil B ENDGÜLTIG (Nachzügler), ADR-062 (supersedes ADR-061 Teil B). User wies an, das vertagte Teil B zu lösen. Vertiefte Analyse MAXential (18 Gedanken, Branches rebuild/reframe verworfen) + ToT (entfernen 0.92) + **IL-Live-Probe**: Method-Body byte-identisch mit/ohne `where T:class` (Reflection, 4==4 Bytes) → Generic Constraints sind rein Compile-Time, erzeugen kein IL → eine Constraint-Mutation kann NIE killbar sein (nur CompileError oder äquivalenter Survivor), zudem redundant zu Body-Mutatoren (Operator-Mutatoren decken die constraint-relevante Body-Logik ab). **Entscheidung: ENTFERNEN** (statt deaktivieren 0.65 / rebuild 0.10 / status-quo) — `GenericConstraintMutator` + `GenericConstraintLoosenMutator` + 2 Test-Dateien gelöscht (git rm), 2 Orchestrator-Registrierungen + 2 Fremd-crefs (AsSpanAsMemory/TaskWhenAllToWhenAny, sonst CS1574) + 1 RoslynHelper-Kommentar bereinigt. Katalog **52→50** (Defaults 26, Stronger 18→17, All-only 8→7). Build 0/0, Core 562/562 (+Flaky CommentParser isoliert grün), Dogfood 1239/1248, Semgrep 0, MutatorReflectionProperties dynamisch → grün. **🏁 INJ-001 ENDGÜLTIG GESCHLOSSEN** (Teil A S187, Teil B S188); #279-Epic von Constraint-Altlast befreit. Lehre: ein nicht-injizierbarer Mutator ist kein zu-fixender Bug, wenn das Ziel semantisch keinen killbaren Mutanten erzeugen KANN — dann ist Entfernen der Fix, nicht Engineering.
