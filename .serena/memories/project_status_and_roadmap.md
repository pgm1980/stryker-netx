...
| 182 | Robustheit | **IN PROGRESS (Branch feature/182-robustness, gestartet 2026-06-12)**: #277a (RegexMutator.cs:42 harter LiteralExpressionSyntax-Cast nach breitem IsAStringExpression → is-Pattern, interpolierte unmutiert), #277b (try/catch-Telemetrie um mutator.Mutate in CsharpMutantOrchestrator.GenerateMutationsForNode — Mutator-Bug darf Lauf nie töten; In-Repo-Präzedenz CommentParser-Timeout-Catch), #288 (CsharpCompilingProcess.TryCompilation:181 while(emitResult==null) ohne Bound — Abbruch wenn ScanForCauseOfException Trees nicht ändert → CompilationException), #297a (VsTestRunnerPool.Initialize fire-and-forget → MTP-Pool-Muster spiegeln), #297b (DiscoveryEventHandler.WaitEnd Monitor.Wait ohne Timeout), #297c (TestingPlatformClient.Disconnected komplettiert ResponseListener nicht → TrySetException), Parse-Guards H-19 (IProjectAnalysisExtensions bool/int.Parse, LoadReferencesIterator Existenz-Guard, SolutionFile-IOE ins Resolver-Catch-Set) + G-37b (BaselineMutantFilter Enum.TryParse + Skip). Erfolgsmaß: Crash-Probe interpolierter Regex (Level Advanced) → Exit 0 + WARN |
