...
public class AzureFileStorageSasInputTests
{
    ...
    public void ShouldHaveHelpText()
    {
        ...
        target.HelpText.Should().Be("A Shared Access Signature for Azure File Storage is required when Azure File Storage is used for dashboard compare. For more information: https://docs.microsoft.com/en-us/azure/storage/common/storage-sas-overview | default: ''");
    }
    ...
    public void Should_Accept_Valid_SAS()
    {
        ...
        validatedSas.Should().Be(ValidSasInput);
    }
    ...
    [InlineData("se=2022-08-27T09%3A26%3A07Z&sp=rwdl&spr=https&sv=2018-11-09&sr=s&")]
    public void Should_Throw_Exception_When_Missing_Important_Keys(string input)
    {
        ...
        act.Should().Throw<InputException>()
            .WithMessage("The azure file storage shared access signature is not in the correct format");
    }
    ...
    public void Should_Throw_Exception_When_AzureSAS_nullOrWhitespace(string? input)
    {
        ...
        var act = () => target.Validate(BaselineProvider.AzureFileStorage, true);
        act.Should().Throw<InputException>()
            .WithMessage("The azure file storage shared access signature is required when azure file storage is used for dashboard compare.");
    }
}
