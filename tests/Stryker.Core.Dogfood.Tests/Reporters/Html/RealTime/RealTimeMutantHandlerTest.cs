...
/// <summary>Sprint 82 (v2.68.0) port. MSTest → xUnit, Shouldly → FluentAssertions.
/// Inherits TestBase: RealTimeMutantHandler ctor uses ApplicationLogging.LoggerFactory.</summary>
public class RealTimeMutantHandlerTest : TestBase
{
    ...
    public void ShouldQueueEventsUntilAtleastOneClientIsConnected()
    {
        ...
        _sseEventSenderMock.Verify(sse => sse.SendEvent(It.IsAny<SseEvent<JsonMutant>>()));
    }

    private static Mutant BuildMutant() => new()
    {
        Id = 1,
