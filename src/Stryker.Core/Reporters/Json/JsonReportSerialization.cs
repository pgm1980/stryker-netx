...
public static class JsonReportSerialization
{
    ...
    public static async Task<IJsonReport?> DeserializeJsonReportAsync(this Stream stream) => await JsonSerializer.DeserializeAsync<JsonReport>(stream, Options).ConfigureAwait(false);

    public static async Task SerializeAsync(this IJsonReport report, Stream stream) => await JsonSerializer.SerializeAsync(stream, report, Options).ConfigureAwait(false);
