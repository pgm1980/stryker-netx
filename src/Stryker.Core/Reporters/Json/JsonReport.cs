...
public class JsonReport : IJsonReport
{
    ...
    public IDictionary<string, int> Thresholds { get; init; } = new Dictionary<string, int>(StringComparer.Ordinal);
    public string ProjectRoot { get; init; } = string.Empty;
    public IDictionary<string, ISourceFile> Files { get; init; } = new Dictionary<string, ISourceFile>(StringComparer.Ordinal);
    public IDictionary<string, IJsonTestFile>? TestFiles { get; set; }
    ...
    {
        var files = new Dictionary<string, ISourceFile>(StringComparer.Ordinal);
    ...
    private static Dictionary<string, ISourceFile> GenerateFileReportComponents(IStrykerOptions options, IReadOnlyFileLeaf fileComponent)
    {
        ...
        {
            return new Dictionary<string, ISourceFile>(StringComparer.Ordinal) { { fileComponent.FullPath, SourceFile.Ignored } };
        }
        return new Dictionary<string, ISourceFile>(StringComparer.Ordinal) { { fileComponent.FullPath, new SourceFile(fileComponent) } };
     }
    ...
    private void AddTestFiles(ITestProjectsInfo testProjectsInfo)
    {
        ...
        {
            TestFiles = new Dictionary<string, IJsonTestFile>(StringComparer.Ordinal);
