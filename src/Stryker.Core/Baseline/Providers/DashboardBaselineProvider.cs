...
public sealed class DashboardBaselineProvider : IBaselineProvider
{
    ...
    {
        return await _client.PullReport(version).ConfigureAwait(false);
    }

    public async Task Save(IJsonReport report, string version)
    {
        await _client.PublishReport(report, version).ConfigureAwait(false);
    }
}
