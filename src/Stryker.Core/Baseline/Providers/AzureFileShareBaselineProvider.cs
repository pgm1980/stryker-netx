...
public class AzureFileShareBaselineProvider : IBaselineProvider
{
    ...
    public AzureFileShareBaselineProvider(IStrykerOptions options, ShareClient? shareClient = null, ILogger<AzureFileShareBaselineProvider>? logger = null)
    {
        ...
        _fileShareClient = shareClient ?? new ShareClient(new Uri(options.AzureFileStorageUrl ?? string.Empty), new AzureSasCredential(options.AzureFileStorageSas ?? string.Empty));
    }
    ...
    public async Task<IJsonReport?> Load(string version)
    {
        ...
        if (TryGetAuthenticatedClient(root, out var directoryClient) && directoryClient is not null)
        {
            if (TryEnumerateDirectories(directoryClient, subdirectoryNames, createIfNotExists: false, out directoryClient))
            {
                ...
                {
                    var downloadResponse = await fileClient.DownloadAsync().ConfigureAwait(false);
                    return await downloadResponse.Value.Content.DeserializeJsonReportAsync().ConfigureAwait(false);
                }
            }
        }
        else
    ...
    public async Task Save(IJsonReport report, string version)
    {
        ...
        if (TryGetAuthenticatedClient(root, out var directoryClient) && directoryClient is not null)
        {
            if (TryEnumerateDirectories(directoryClient, subdirectoryNames, createIfNotExists: true, out directoryClient))
            {
                ...
                try
                {
                    await fileClient.CreateAsync(fileContentStream.Length).ConfigureAwait(false);
                    await UploadFileContent(uri, fileClient, fileContentStream).ConfigureAwait(false);
                }
    ...
    private static bool TryEnumerateDirectories(ShareDirectoryClient directoryClient, string[] subdirectories, bool createIfNotExists, out ShareDirectoryClient? shareDirectoryClient)
    {
    ...
    private async Task UploadFileContent(string uri, ShareFileClient fileClient, MemoryStream fileContentStream)
    {
        ...
        while (true)
        {
            ...
            await uploadChunk.WriteAsync(buffer.AsMemory(0, buffer.Length)).ConfigureAwait(false);
