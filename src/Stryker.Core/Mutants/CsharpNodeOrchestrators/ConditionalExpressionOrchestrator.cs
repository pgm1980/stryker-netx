using Microsoft.CodeAnalysis;
using Microsoft.CodeAnalysis.CSharp.Syntax;
using Stryker.Core.Mutants;

namespace Stryker.Core.Mutants.CsharpNodeOrchestrators;
internal sealed class ConditionalExpressionOrchestrator : MemberAccessExpressionOrchestrator<ConditionalAccessExpressionSyntax>
{

    // Sprint 151 (ADR-032): routed through OrchestrationHelpers.ReplaceChildrenValidated
    // for per-child slot-validation. See NodeSpecificOrchestrator.OrchestrateChildrenMutation
    // remarks for details on the validation strategy.
    protected override ExpressionSyntax OrchestrateChildrenMutation(ConditionalAccessExpressionSyntax node, SemanticModel semanticModel,
        MutationContext context) =>
        OrchestrationHelpers.ReplaceChildrenValidated(node, node.ChildNodes(),
            original =>
            {
                if (original != node.WhenNotNull)
                {
                    return context.Mutate(original, semanticModel);
                }
                // there must be some MemberBindingAccess in the chain, so we assume we are in a member access chain (no mutation injection)
                var subContext = context.Enter(MutationControl.MemberAccess);
                var result = subContext.Mutate(original, semanticModel);
                subContext.Leave();
                return result;
            });
}
