...
public class MutantPlacer
{
    ...
    public StatementSyntax PlaceStatementControlledMutations(StatementSyntax original,
        ...
        mutants.Aggregate(original, (syntaxNode, mutationInfo) =>
            IfEngine.InjectIf(GetBinaryExpression(mutationInfo.mutant.Id), syntaxNode, mutationInfo.mutation)
                ...
                .WithAdditionalAnnotations(new SyntaxAnnotation(MutationIdMarker, mutationInfo.mutant.Id.ToString(CultureInfo.InvariantCulture)))
    ...
    public static bool RequiresRemovingChildMutations(SyntaxNode node)
    {
        ...
        if (annotations.TrueForAll(a => !string.Equals(a.Kind, Injector, StringComparison.Ordinal)))
        {
            throw new InvalidOperationException("No mutation in this node!");
        }
        return annotations.Exists(a => a.Data is not null && requireRecursiveRemoval.Contains(a.Data));
    }
    ...
    public static MutantInfo FindAnnotations(SyntaxNode node)
    {
        ...
        {
            if (string.Equals(annotation.Kind, MutationIdMarker, StringComparison.Ordinal))
            {
                id = int.Parse(annotation.Data!, CultureInfo.InvariantCulture);
            }
            else if (string.Equals(annotation.Kind, Injector, StringComparison.Ordinal))
            {
                engine = annotation.Data;
            }
            else if (string.Equals(annotation.Kind, MutationTypeMarker, StringComparison.Ordinal))
            {
