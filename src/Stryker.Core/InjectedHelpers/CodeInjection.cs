...
public class CodeInjection
{
    ...
    private const string PatternForCheck = "\\/\\/ *check with: *(?<selector>[^\\r\\n]+)";
    private const string MutantContextClassName = "MutantContext";
    private const string StrykerNamespace = "Stryker";
    private static readonly string Selector = ExtractSelector();

    private static string ExtractSelector()
    {
        var helper = GetSourceFromResource("Stryker.Core.InjectedHelpers.MutantControl.cs");
        var extractor = new Regex(PatternForCheck, RegexOptions.ExplicitCapture, TimeSpan.FromMilliseconds(200));
        var result = extractor.Match(helper);
        if (!result.Success)
        {
            throw new InvalidOperationException("Internal error: failed to find expression for mutant selection.");
        }

        return result.Groups["selector"].Value;
    }
    ...
    public IDictionary<string, string> MutantHelpers { get; } = new Dictionary<string, string>(StringComparer.Ordinal);
    ...
    public static bool IsContextAccessExpression(ExpressionSyntax memberAccess, string member) =>
        ...
        } && string.Equals(specificMember, member, StringComparison.Ordinal);
