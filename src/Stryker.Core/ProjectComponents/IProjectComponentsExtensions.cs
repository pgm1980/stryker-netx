...
public static class IProjectComponentsExtensions
{
    ...
    public static bool IsComponentExcluded(this IReadOnlyProjectComponent projectComponent, IEnumerable<IFilePattern> filePatterns)
    {
        ...
        if (includedSpans.Count > 0 && excludedSpans.Count == 0 || projectComponent.Parent?.Parent == null)
        {
            return false;
        }

        return includedSpans.RemoveOverlap(excludedSpans).Count == 0;

        bool MatchesFilePattern(IFilePattern pattern) =>
