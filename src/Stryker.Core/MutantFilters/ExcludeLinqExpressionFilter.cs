...
public class ExcludeLinqExpressionFilter : IMutantFilter
{
    ...
    private static bool MatchesAnIgnoredExpression(string expressionString, IStrykerOptions options) => options.ExcludedLinqExpressions.Any(r => expressionString.EndsWith(Enum.GetName(r) ?? string.Empty, StringComparison.Ordinal));
