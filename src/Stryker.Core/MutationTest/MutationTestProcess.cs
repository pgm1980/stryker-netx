...
public class MutationTestProcess : IMutationTestProcess
{
    ...
    {
        var testsFailingInitially = Input.InitialTestRun.Result.FailingTests.GetIdentifiers().ToHashSet(StringComparer.Ordinal);
    ...
    private IEnumerable<List<IMutant>> BuildMutantGroupsForTest(List<IMutant> mutantsNotRun)
    {
        ...
        var blocks = BuildOptimizedMutantBlocks(mutantsNotRun);
    ...
    private List<List<IMutant>> BuildOptimizedMutantBlocks(List<IMutant> mutantsNotRun)
    {
        ...
        while (mutantsToGroup.Count > 0)
        {
            ...
            while (j < mutantsToGroup.Count)
            {
                ...
                // remove the mutant from the list of mutants to group (do not advance j; same index now points to next item)
