...
public partial class CoverageAnalyser : ICoverageAnalyser
{
        ...
        TestIdentifierList failedTests)
    {
        ...
        var (mutationToResultMap, testIds, trustedTests, dubiousTests) = AggregateCoverage(coverage, failedTests);
        ...
        var allTestsExceptTrusted = trustedTests.Count == 0 && failedTests.Count == 0
            ...
            : new TestIdentifierList(testIds.Except(trustedTests, StringComparer.Ordinal).ToHashSet(StringComparer.Ordinal)).Excluding(failedTests);
    ...
    private static (Dictionary<int, List<ICoverageRunResult>> mutationToResultMap, HashSet<string> testIds, HashSet<string> trustedTests, HashSet<string> dubiousTests)
        AggregateCoverage(IEnumerable<ICoverageRunResult> coverage, TestIdentifierList failedTests)
    {
        var dubiousTests = new HashSet<string>(StringComparer.Ordinal);
        var trustedTests = new HashSet<string>(StringComparer.Ordinal);
        ...
        foreach (var coverageRunResult in coverage)
        {
            foreach (var i in coverageRunResult.MutationsCovered)
            {
                ...
                resultList.Add(coverageRunResult);
            }

            if (failedTests.GetIdentifiers().Contains(coverageRunResult.TestId, StringComparer.Ordinal))
            {
        ...
        return (mutationToResultMap, testIds, trustedTests, dubiousTests);
    }
