...
public class CoverageCollector : InProcDataCollection, IDisposable
{
    ...
    private readonly Dictionary<string, int> _mutantTestedBy = new(StringComparer.Ordinal);
...
<Configuration>{1}</Configuration></InProcDataCollector></InProcDataCollectors></InProcDataCollectionRunSettings>";
        ...
        string helperNameSpace)
    {
        ...
        var friendlyAttribute = typeof(CoverageCollector).ExtractAttribute<DataCollectorFriendlyNameAttribute>();
        var friendlyName = friendlyAttribute?.FriendlyName ?? string.Empty;
        var uriAttribute = typeof(CoverageCollector).GetTypeInfo()
            .GetCustomAttributes(typeof(DataCollectorTypeUriAttribute), false)
            .FirstOrDefault() as DataCollectorTypeUriAttribute;
        var uri = uriAttribute?.TypeUri ?? string.Empty;
        var line =
        ...
        if (mutantTestsMap != null)
        {
            ...
            {
                configuration.AppendFormat(CultureInfo.InvariantCulture, "<Mutant id='{0}' tests='{1}'/>", mutant,
                    coveringTests == null ? string.Empty : string.Join(",", coveringTests));
            }
        }
        ...
        return string.Format(CultureInfo.InvariantCulture, TemplateForConfiguration, line, configuration);
    }
    ...
    private void FindControlType(Assembly assembly)
    {
        ...
        _mutantControlType = assembly.DefinedTypes?.FirstOrDefault(t => string.Equals(t.FullName, _controlClassName, StringComparison.Ordinal));
        if (_mutantControlType == null)
        {
            return;
        }
    ...
    private void ParseTestMapping(XmlNodeList testMapping)
    {
        ...
        for (var i = 0; i < testMapping.Count; i++)
        {
            ...
            var idAttr = current.Attributes["id"]?.Value;
            var tests = current.Attributes["tests"]?.Value;
            if (idAttr is null)
            {
                continue;
            }
            var id = int.Parse(idAttr, CultureInfo.InvariantCulture);
            mutations.Add(id);
    ...
    public void TestCaseStart(TestCaseStartArgs testCaseStartArgs)
    {
        ...
        Log($"Test {testCase.FullyQualifiedName ?? "<unknown>"} starts against mutant {_activeMutation} (var).");
    }

    public void TestCaseEnd(TestCaseEndArgs testCaseEndArgs)
    {
        Log($"Test {testCaseEndArgs.DataCollectionContext.TestCase?.FullyQualifiedName ?? "<unknown>"} ends.");
        if (!_coverageOn)
        {
            return;
        }
    ...
    private void PublishCoverageData(DataCollectionContext dataCollectionContext)
    {
        ...
        if (covered == null)
        {
            ...
            _dataSink?.SendData(dataCollectionContext, CoverageLog, $"Test {dataCollectionContext.TestCase?.DisplayName ?? "<unknown>"} endend. No mutant covered so far.");
            return;
        }
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (disposing)
        {
            _throwingListener?.Dispose();
            _throwingListener = null;
        }
    }
