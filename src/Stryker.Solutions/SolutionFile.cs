...
public class SolutionFile
{
    ...
    public IReadOnlyCollection<string> GetBuildTypes() =>
        _configurations.Keys.Select(x => x.buildType).ToHashSet(StringComparer.Ordinal);

    public string FileName { get; init; } = string.Empty;

    private HashSet<string> GetPlatforms() =>
        _configurations.Keys.Select(x => x.platform).ToHashSet(StringComparer.Ordinal);
    ...
    public bool ConfigurationExists(string buildType, string? platform = null)
    {
        ...
        return _configurations.Keys.Any(x =>
            string.Equals(x.buildType, buildType, StringComparison.Ordinal) &&
            string.Equals(platform, x.platform, StringComparison.Ordinal));
    }
    ...
    /// <returns>The best matching solution build type and platform</returns>
    ...
    public (string buildType, string platform) GetMatching(string? buildType, string? platform = null)
    {
        ...
        {
            if (string.Equals(buildType, currentBuildType, StringComparison.Ordinal) &&
                string.Equals(currentPlatform, platform, StringComparison.Ordinal))
            {
                // perfect match
            ...
            if (buildType != null && string.Equals(buildType, currentBuildType, StringComparison.Ordinal))
            {
                // better match
                thisScore = 8;
            }
            else if (string.Equals(currentBuildType, DefaultBuildType, StringComparison.Ordinal))
            {
                thisScore = 4;
            }
            else
            ...
            if (platform != null && string.Equals(platform, currentPlatform, StringComparison.Ordinal))
            {
                // better match
                thisScore += 2;
            }
            else if (string.Equals(currentPlatform, DefaultPlatform, StringComparison.Ordinal))
            {
                thisScore += 1;
            }
    ...
    public static SolutionFile BuildFromProjectList(IReadOnlyCollection<string> projects, string[]? platforms = null)
    {
        ...
        foreach (var buildType in buildTypes)
        {
            ...
            {
                var projectDict = new Dictionary<string, (string buildType, string platform)>(StringComparer.Ordinal);
    ...
    private static SolutionFile AnalyzeSolution(string solutionPath, SolutionModel solution)
    {
        ...
        foreach (var buildType in solution.BuildTypes)
        {
            ...
            {
                var projects = new Dictionary<string, (string buildType, string platform)>(StringComparer.Ordinal);
