...
public sealed class VsTestRunner : IDisposable
{
    ...
    /// <summary>Extra timeout (in milliseconds) added to VsTest wrapper operations to detect hangs.</summary>
    ...
    private string RunnerId => $"Runner {_id.ToString(CultureInfo.InvariantCulture)}";
    private string ControlVariableName => $"ACTIVE_MUTATION_{_id.ToString(CultureInfo.InvariantCulture)}";

    /// <summary>Initializes a new <see cref="VsTestRunner"/>.</summary>
    ...
    /// <summary>Runs the initial test session and registers per-test timing data.</summary>
    public ITestRunResult InitialTest(IProjectAndTests project)
    {
        ...
        {
            if (!_context.VsTests.TryGetValue(result.TestCase.Id, out var description))
            {
    ...
    /// <summary>Tests the supplied mutants and reports results to the optional <paramref name="update"/> callback.</summary>
    public ITestRunResult TestMultipleMutants(IProjectAndTests project, ITimeoutValueCalculator? timeoutCalc, IReadOnlyList<IMutant> mutants, TestUpdateHandler? update)
    {
        ...
        var testResults = RunTestSession(new TestIdentifierList(testCases ?? []), project, timeOutMs, mutantTestsMap, ctx.HandleUpdate);

        return BuildTestRunResult(testResults, expectedTests, totalCountOfTests);
    }

    private int? ResolveTimeout(ITimeoutValueCalculator? timeoutCalc, ICollection<string>? testCases)
    {
    ...
    private sealed class MutantTestSessionContext
    {
        private readonly VsTestRunner _runner;
        private readonly IReadOnlyList<IMutant> _mutants;
        private readonly int _totalCountOfTests;
        private readonly int _expectedTests;
        private readonly TestUpdateHandler? _update;

        public MutantTestSessionContext(VsTestRunner runner, IReadOnlyList<IMutant> mutants, int totalCountOfTests, int expectedTests, TestUpdateHandler? update)
        {
            _runner = runner;
            _mutants = mutants;
            _totalCountOfTests = totalCountOfTests;
            _expectedTests = expectedTests;
            _update = update;
        }

        public void HandleUpdate(IRunResults handler)
        {
            var handlerTestResults = handler.TestResults;
            var tests = handlerTestResults.Select(p => p.TestCase.Id).Distinct().Count() >= _totalCountOfTests
            ...
            var remainingMutants = _update?.Invoke(_mutants, failedTest, tests, timedOutTest);

            if (remainingMutants != false
                || handlerTestResults.Count >= _expectedTests
                || _runner._currentSessionCancelled
                || _runner._context.Options.OptimizationMode.HasFlag(OptimizationModes.DisableBail))
            {
                return;
            }
            ...
            bool compressAll = true)
    {
        ...
        var testCases = resultAsArray.Select(t => t.TestCase.Id.ToString()).ToHashSet(StringComparer.Ordinal);
    ...
    /// <summary>Runs an aggregated test session (non-coverage path) and returns normalized results.</summary>
    public IRunResults RunTestSession(ITestIdentifiers testsToRun, IProjectAndTests project, int? timeout = null, IDictionary<int, ITestIdentifiers>? mutantTestsMap = null, Action<IRunResults>? updateHandler = null) =>
        RunTestSession(testsToRun, project, false, timeout, updateHandler, mutantTestsMap).normal;

    /// <summary>Runs a coverage-collection session and returns the raw VsTest results.</summary>
    ...
    private (IRunResults normal, IRunResults raw) RunTestSession(ITestIdentifiers tests, IProjectAndTests projectAndTests,
        bool forCoverage, int? timeOut = null,
        // S1172: parameter is captured by the local HandlerUpdate function below; the analyzer does not detect closure-capture.
#pragma warning disable S1172 // Unused method parameter — captured in HandlerUpdate local function
...
#pragma warning restore S1172
        IDictionary<int, ITestIdentifiers>? mutantTestsMap = null)
    {
        ...
        foreach (var source in projectAndTests.TestProjectsInfo.Analyses)
        {
            ...
            Environment.SetEnvironmentVariable(ControlVariableName, activeId.ToString(CultureInfo.InvariantCulture));
    ...
    private void PrepareVsTestConsole()
    {
        ...
        _vsTestConsole = _context.BuildVsTestWrapper($"{RunnerId}-{_instanceCount.ToString(CultureInfo.InvariantCulture)}", ControlVariableName);
    }
