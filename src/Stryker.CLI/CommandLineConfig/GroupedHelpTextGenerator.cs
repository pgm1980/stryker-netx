...
internal sealed class GroupedHelpTextGenerator : DefaultHelpTextGenerator
{
        ...
        if (visibleOptions.Count > 0)
        {
            ...
            foreach (var group in visibleCategorizedOptions.Cast<StrykerInputOption>().GroupBy(c => c.Category).OrderBy(g => g.Key))
            {
                ...
                foreach (var opt in group)
                {
                    ...
                    var message = string.Format(CultureInfo.InvariantCulture, outputFormat, Format(opt), wrappedDescription);
