...
public sealed class LoggingInitializer : ILoggingInitializer
{
    /// <summary>
    /// Creates the needed paths for logging and initializes the logger factory
    /// </summary>
    /// <param name="inputs">Stryker inputs containing log/output configuration</param>
    ...
    {
        var outputPath = inputs.OutputPathInput.SuppliedInput ?? Path.Combine("StrykerOutput", DateTime.Now.ToString("yyyy-MM-dd.HH-mm-ss", CultureInfo.InvariantCulture));

        if (!Path.IsPathRooted(outputPath))
        {
            outputPath = Path.Combine(inputs.BasePathInput.SuppliedInput ?? string.Empty, outputPath);
        }
