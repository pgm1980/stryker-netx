...
public static class FileConfigGenerator
{
        ...
        return new FileBasedInputOuter
        {
            Input = new FileBasedInput
            {
                Baseline = new Baseline
                {
                    ...
                    S3Endpoint = inputs.S3EndpointInput.SuppliedInput ?? inputs.S3EndpointInput.Default,
                    S3Region = inputs.S3RegionInput.SuppliedInput ?? inputs.S3RegionInput.Default,
                },
                Since = new Since
                {
                    Enabled = inputs.SinceInput.SuppliedInput ?? inputs.SinceInput.Default,
                    IgnoreChangesIn = inputs.DiffIgnoreChangesInput.SuppliedInput is null ? [.. inputs.DiffIgnoreChangesInput.Default] : [.. inputs.DiffIgnoreChangesInput.SuppliedInput],
                ...
                TestProjects = inputs.TestProjectsInput.SuppliedInput is null ? [.. inputs.TestProjectsInput.Default] : [.. inputs.TestProjectsInput.SuppliedInput],
                ...
                IgnoreMutations = inputs.IgnoreMutationsInput.SuppliedInput is null ? [.. inputs.IgnoreMutationsInput.Default] : [.. inputs.IgnoreMutationsInput.SuppliedInput],
    ...
    private static string SerializeConfig(FileBasedInputOuter config) => JsonSerializer.Serialize(config, SerializerOptions);
}
