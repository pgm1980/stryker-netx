...
public sealed partial class StrykerNugetFeedClient : IStrykerNugetFeedClient, IDisposable
{
    ...
    public async Task<SemanticVersion> GetLatestVersionAsync() => await GetVersionAsync(prerelease: false).ConfigureAwait(false);

    public async Task<SemanticVersion> GetPreviewVersionAsync() => await GetVersionAsync(prerelease: true).ConfigureAwait(false);
