...
public class CoverageAnalysisInput : Input<string>
{
    ...
    protected override string Description
    {
        get
        {
            ...
            result.AppendJoin(", ", _possibleValues.Keys).AppendLine(CultureInfo.InvariantCulture, $".");
            foreach (var possibleValue in _possibleValues)
            {
                result.AppendLine(CultureInfo.InvariantCulture, $"\t- {possibleValue.Key}: {possibleValue.Value.description}");
            }
