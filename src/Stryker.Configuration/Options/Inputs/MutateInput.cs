...
public class MutateInput : Input<IEnumerable<string>>
{
    ...
    public override IEnumerable<string> Default => [_defaultInput];
    ...
    public IEnumerable<IFilePattern> Validate()
    {
        if (SuppliedInput is { } && SuppliedInput.Any())
        {
            ...
            {
                var normalized = FilePathUtils.NormalizePathSeparators(pattern) ?? pattern;
                filesToInclude.Add(FilePattern.Parse(normalized));
            }
