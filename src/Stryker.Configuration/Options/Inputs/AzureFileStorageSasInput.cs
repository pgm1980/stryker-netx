...
public class AzureFileStorageSasInput : Input<string>
{
    ...
    public string Validate(BaselineProvider baselineProvider, bool withBaseline)
    {
        if (withBaseline && baselineProvider == BaselineProvider.AzureFileStorage)
        {
            ...
            var hasImportantKey = query.AllKeys.Where(x => x is not null && (string.Equals(x, "sv", StringComparison.OrdinalIgnoreCase) || string.Equals(x, "sig", StringComparison.OrdinalIgnoreCase)));
