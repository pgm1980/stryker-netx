...
public class OpenReportInput : Input<string>
{
    ...
    protected override IEnumerable<string> AllowedOptions => Enum.GetNames<ReportType>();
    ...
    public ReportType? Validate(bool enabled)
    {
        if (enabled)
        {
            ...
            else
            {
                ...
                {
                    throw new InputException($"The given report type ({SuppliedInput}) is invalid. Valid options are: [{string.Join(", ", Enum.GetValues<ReportType>())}]");
                }
            }
        }
