...
public class WrappedIdentifierEnumeration : ITestIdentifiers
{
    ...
    public bool Contains(string testId) => _identifiers.Any(g => string.Equals(g, testId, StringComparison.Ordinal));
    ...
    public static ITestIdentifiers MergeList(ITestIdentifiers a, ITestIdentifiers b)
    {
        ...
        return b.GetIdentifiers() is null ? a : new WrappedIdentifierEnumeration(a.GetIdentifiers().Union(b.GetIdentifiers(), StringComparer.Ordinal));
    }

    public IEnumerable<string> GetIdentifiers() => _identifiers;

    public ITestIdentifiers Intersect(ITestIdentifiers other) => IsEveryTest ? new WrappedIdentifierEnumeration(other.GetIdentifiers()) : new WrappedIdentifierEnumeration(_identifiers.Intersect(other.GetIdentifiers(), StringComparer.Ordinal));

    public ITestIdentifiers Excluding(ITestIdentifiers testsToSkip) => throw new NotSupportedException("Excluding from a wrapped identifier enumeration is not supported.");
}
