...
public class TestIdentifierList : ITestIdentifiers
{
    ...
    public TestIdentifierList(IEnumerable<string> identifiers) => _identifiers = identifiers != null ? new HashSet<string>(identifiers, StringComparer.Ordinal) : null;

    public TestIdentifierList(IReadOnlyCollection<string> identifiers) => _identifiers = new HashSet<string>(identifiers, StringComparer.Ordinal);
    ...
    public bool ContainsAny(ITestIdentifiers other)
    {
        ...
        return GetIdentifiers().Intersect(other.GetIdentifiers(), StringComparer.Ordinal).Any();
    }
    ...
    public ITestIdentifiers Merge(ITestIdentifiers other)
    {
        ...
        var result = new HashSet<string>(_identifiers ?? [], StringComparer.Ordinal);
        result.UnionWith(other.GetIdentifiers());
        return new TestIdentifierList((IReadOnlyCollection<string>)result);
    }
    ...
    public ITestIdentifiers Excluding(ITestIdentifiers testsToSkip)
    {
        ...
        return testsToSkip.IsEveryTest ? NoTest() : new TestIdentifierList((_identifiers ?? Enumerable.Empty<string>()).Except(testsToSkip.GetIdentifiers(), StringComparer.Ordinal));
    }
}
